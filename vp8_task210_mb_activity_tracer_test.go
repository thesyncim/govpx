...
// TestVP8Task210MBActivityTracerSeeds drives the two residual 1280x720
...
//	  go test -tags govpx_oracle_trace -run TestVP8Task210MBActivityTracerSeeds -v
//
...
func TestVP8Task210MBActivityTracerSeeds(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// Matches TestVP8Byte0KF1280x720SSIMBestARNRAudit (task #207):
	...
	for _, c := range cases {
		...
		t.Run(c.name, func(t *testing.T) {
			runTask210MBActivityProbe(t, vpxencOracle, c.seedHash, c.opts, c.targetKbps, c.extra)
		})
	}
}

func runTask210MBActivityProbe(t *testing.T, vpxencOracle string, seedHash string, opts EncoderOptions, targetKbps int, extraArgs []string) {
	t.Helper()
