...
func TestEncodeInterFrameAttemptSuppressesAltRefCopyBufferOnHiddenARF(t *testing.T) {
	...
	attempt, err := e.encodeInterFrameAttempt(dst, sourceImageFromImage(testImage(e.opts.Width, e.opts.Height)), rows, cols, required, flags, false, false, false)
	if err != nil {
...
func TestEncodeInterFrameAttemptClearsCopyBuffersOnDeferredShowFrame(t *testing.T) {
	...
	attempt, err := e.encodeInterFrameAttempt(dst, sourceImageFromImage(testImage(e.opts.Width, e.opts.Height)), rows, cols, required, 0, false, true, false)
	if err != nil {
