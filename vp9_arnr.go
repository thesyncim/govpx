...
func applyVP9TemporalFilter16(srcY, predY, srcU, predU, srcV, predV []byte,
	strength int, filterWeight int,
	yAccumulator, yCount, uAccumulator, uCount, vAccumulator, vCount []uint32,
) {
	if filterWeight <= 0 {
		return
	...
	var yDiff [256]int
	var uDiff, vDiff [64]int
	for y := range 16 {
		for x := range 16 {
			diff := int(srcY[y*16+x]) - int(predY[y*16+x])
			yDiff[y*16+x] = diff * diff
		}
	}
	for y := range 8 {
		for x := range 8 {
			u := int(srcU[y*8+x]) - int(predU[y*8+x])
			v := int(srcV[y*8+x]) - int(predV[y*8+x])
			uDiff[y*8+x] = u * u
			vDiff[y*8+x] = v * v
		}
	}
	for y := range 16 {
		for x := range 16 {
			...
			uSum += ySum
			vSum += ySum
			uMod := vp9TemporalFilterModIndex(uSum, chromaUsed, rounding,
				strength, filterWeight)
			vMod := vp9TemporalFilterModIndex(vSum, chromaUsed, rounding,
				strength, filterWeight)
			uv := uvY*8 + uvX
			uCount[uv] += uint32(uMod)
			uAccumulator[uv] += uint32(uMod) * uint32(predU[uv])
