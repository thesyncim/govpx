package govpx_test

import (
	"errors"
	"testing"

	govpx "github.com/thesyncim/govpx"
)

func TestVP9EncoderAcceptsSourceExceedingTargetLevelGeometry(t *testing.T) {
	opts := govpx.VP9EncoderOptions{
		Width:       3840,
		Height:      2160,
		FPS:         60,
		TargetLevel: 31,
	}
	e, err := govpx.NewVP9Encoder(opts)
	if err != nil {
		t.Fatalf("4K@60 with TargetLevel=31 err = %v, want nil", err)
	}
	_ = e.Close()
}

func TestVP9EncoderAcceptsFrameWithinTargetLevel(t *testing.T) {
	// 720p@30 fits inside level 3.1's 36864000 sample/sec, 983040 luma
	// picture-size budget.
	opts := govpx.VP9EncoderOptions{
		Width:       1280,
		Height:      720,
		FPS:         30,
		TargetLevel: 31,
	}
	e, err := govpx.NewVP9Encoder(opts)
	if err != nil {
		t.Fatalf("720p@30 with TargetLevel=31 err = %v, want nil", err)
	}
	_ = e.Close()
}

func TestVP9EncoderAcceptsBitrateExceedingTargetLevelAverage(t *testing.T) {
	opts := govpx.VP9EncoderOptions{
		Width:              640,
		Height:             480,
		FPS:                30,
		TargetLevel:        30,
		TargetBitrateKbps:  30000,
		RateControlModeSet: true,
		RateControlMode:    govpx.RateControlVBR,
	}
	e, err := govpx.NewVP9Encoder(opts)
	if err != nil {
		t.Fatalf("bitrate above level average err = %v, want nil", err)
	}
	_ = e.Close()
}

func TestVP9EncoderAcceptsFrameRateExceedingTargetLevelSampleRate(t *testing.T) {
	opts := govpx.VP9EncoderOptions{
		Width:       1920,
		Height:      1080,
		FPS:         60,
		TargetLevel: 40,
	}
	e, err := govpx.NewVP9Encoder(opts)
	if err != nil {
		t.Fatalf("1080p@60 with TargetLevel=40 err = %v, want nil", err)
	}
	_ = e.Close()
}

func TestVP9EncoderTargetLevelAutoSkipsLimitCheck(t *testing.T) {
	opts := govpx.VP9EncoderOptions{
		Width:       3840,
		Height:      2160,
		FPS:         60,
		TargetLevel: govpx.VP9TargetLevelAuto,
	}
	e, err := govpx.NewVP9Encoder(opts)
	if err != nil {
		t.Fatalf("4K@60 with TargetLevel=auto err = %v, want nil", err)
	}
	_ = e.Close()
}

func TestVP9EncoderTargetLevelMaxSkipsLimitCheck(t *testing.T) {
	opts := govpx.VP9EncoderOptions{
		Width:       3840,
		Height:      2160,
		FPS:         60,
		TargetLevel: govpx.VP9TargetLevelUnconstrained,
	}
	e, err := govpx.NewVP9Encoder(opts)
	if err != nil {
		t.Fatalf("4K@60 with TargetLevel=255 err = %v, want nil", err)
	}
	_ = e.Close()
}

func TestVP9EncoderSetTargetLevelAcceptsExceedingConfiguration(t *testing.T) {
	e, err := govpx.NewVP9Encoder(govpx.VP9EncoderOptions{
		Width:  3840,
		Height: 2160,
		FPS:    60,
	})
	if err != nil {
		t.Fatalf("NewVP9Encoder: %v", err)
	}
	defer e.Close()
	if err := e.SetTargetLevel(40); err != nil {
		t.Fatalf("SetTargetLevel(40) err = %v, want nil", err)
	}
	if err := e.SetTargetLevel(62); err != nil {
		t.Fatalf("SetTargetLevel(62) err = %v, want nil", err)
	}
}

func TestVP9EncoderRejectsInvalidTargetLevel(t *testing.T) {
	for _, level := range []int{-1, 2, 5, 12, 100, 254} {
		opts := govpx.VP9EncoderOptions{
			Width:       64,
			Height:      64,
			FPS:         30,
			TargetLevel: level,
		}
		if _, err := govpx.NewVP9Encoder(opts); !errors.Is(err, govpx.ErrInvalidConfig) {
			t.Fatalf("TargetLevel=%d err = %v, want ErrInvalidConfig", level, err)
		}
	}
}

func TestVP9EncoderAcceptsCanonicalTargetLevels(t *testing.T) {
	for _, level := range []int{0, 1, 10, 11, 20, 21, 30, 31, 40, 41, 50, 51, 52, 60, 61, 62, 255} {
		opts := govpx.VP9EncoderOptions{
			Width:       64,
			Height:      64,
			FPS:         30,
			TargetLevel: level,
		}
		e, err := govpx.NewVP9Encoder(opts)
		if err != nil {
			t.Fatalf("TargetLevel=%d unexpected err: %v", level, err)
		}
		_ = e.Close()
	}
}

func TestVP9EncoderSetTargetLevelValidation(t *testing.T) {
	e, err := govpx.NewVP9Encoder(govpx.VP9EncoderOptions{Width: 64, Height: 64, FPS: 30})
	if err != nil {
		t.Fatalf("NewVP9Encoder: %v", err)
	}
	if err := e.SetTargetLevel(31); err != nil {
		t.Fatalf("SetTargetLevel(31): %v", err)
	}
	if err := e.SetTargetLevel(255); err != nil {
		t.Fatalf("SetTargetLevel(255): %v", err)
	}
	if err := e.SetTargetLevel(govpx.VP9TargetLevelAuto); err != nil {
		t.Fatalf("SetTargetLevel(auto): %v", err)
	}
	if err := e.SetTargetLevel(12); !errors.Is(err, govpx.ErrInvalidConfig) {
		t.Fatalf("SetTargetLevel(12) err = %v, want ErrInvalidConfig", err)
	}
	if err := e.SetTargetLevel(-1); !errors.Is(err, govpx.ErrInvalidConfig) {
		t.Fatalf("SetTargetLevel(-1) err = %v, want ErrInvalidConfig", err)
	}
}
