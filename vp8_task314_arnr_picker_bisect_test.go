...
func TestVP8Task314ARNRPickerBisect(t *testing.T) {
	...
	// --- parse + bisect ------------------------------------------------
	govpxBest := collectTask314BecameBest(t, govpxTraceBuf.Bytes())
	libvpxBest := collectTask314BecameBest(t, libvpxTrace)
...
scan:
	...
	govpxAll := collectTask314AllCandidates(t, govpxTraceBuf.Bytes(), fd.MBRow, fd.MBCol)
	libvpxAll := collectTask314AllCandidates(t, libvpxTrace, fd.MBRow, fd.MBCol)
...
func collectTask314BecameBest(t *testing.T, trace []byte) map[task314MBKey]task314Row {
	t.Helper()
...
func collectTask314AllCandidates(t *testing.T, trace []byte, mbRow int, mbCol int) []task314Row {
	t.Helper()
	var out []task314Row
	scan := bufio.NewScanner(bytes.NewReader(trace))
	scan.Buffer(make([]byte, 1<<20), 1<<24)
...
func joinTask314(parts []string) string {
	if len(parts) == 0 {
		return "none"
	}
	out := parts[0]
