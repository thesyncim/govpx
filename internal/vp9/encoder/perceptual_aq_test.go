...
func TestVP9PerceptualHadamard8x8MatchesLibvpx(t *testing.T) {
	...
	var coeff [64]int32
	perceptualHadamard8x8(src[:], 8, coeff[:])
	hits := 0
	for _, c := range coeff {
		switch c {
		case 0:
...
func TestVP9PerceptualHadamard16x16ConstantInput(t *testing.T) {
	...
	var coeff [256]int32
	perceptualHadamard16x16(src[:], 16, coeff[:])
	dc := int32(0)
	nonZero := 0
	for _, c := range coeff {
		if c != 0 {
...
func TestVP9PerceptualKMeansSplitsBimodalDistribution(t *testing.T) {
	...
	var centers, bounds [perceptualAQClusters]float64
	perceptualKMeans(values, &centers, &bounds)
	...
	if centers[perceptualAQClusters-1] < 99.0 {
		t.Fatalf("high-cluster centroid = %g, want >= 99.0; centers=%v",
			centers[perceptualAQClusters-1], centers)
	}
}

func TestVP9PerceptualGroupIndexBoundaries(t *testing.T) {
	bounds := [perceptualAQClusters]float64{
	...
	for _, tc := range cases {
		if got := perceptualGroupIndex(tc.v, &bounds); got != tc.want {
...
func TestVP9PerceptualQIndexQStepRoundtrip(t *testing.T) {
	for q := 4; q < 200; q += 17 {
		step := perceptualQIndexToQStep(q)
		back := perceptualQStepToQIndex(step)
		if back != q {
...
func TestVP9PerceptualSegmentationParamsInterInheritsData(t *testing.T) {
	state := PerceptualAQState{Enabled: true, Ready: true}
	state.deltas[2] = -3
...
//
// ac_quant lookup at base = 32 is perceptualAcQuant8[32] = 39
...
func TestVP9PerceptualAQModeSetupMatchesLibvpxFormula(t *testing.T) {
	...
	wantDeltas := func(i int) int16 {
		...
		idx := 255
		for j := range 256 {
			if float64(perceptualAcQuant8[j])/4.0 >= ts {
				idx = j
				break
			}
		}
	...
	for i := 1; i < perceptualAQClusters; i++ {
...
func TestVP9PerceptualAQModeSetupSignConventionMatchesLibvpx(t *testing.T) {
	centers := []float64{0, 1, 2, 3, 4, 5, 6, 7}
	var deltas [vp9dec.MaxSegments]int16
	perceptualAQModeSetup(centers, 100, deltas[:])
	for i := range perceptualAQClusters / 2 {
		if deltas[i] >= 0 {
			t.Errorf("delta[%d] = %d, want negative (libvpx low-cluster sign)",
				i, deltas[i])
		}
	}
	if deltas[perceptualAQClusters/2] != 0 {
		t.Errorf("delta[mid] = %d, want 0 (libvpx mid-cluster anchor)",
			deltas[perceptualAQClusters/2])
	}
	for i := perceptualAQClusters/2 + 1; i < perceptualAQClusters; i++ {
		if deltas[i] <= 0 {
...
func TestVP9PerceptualKMeansInitMatchesLibvpxQuantilePicks(t *testing.T) {
	...
	var centers, bounds [perceptualAQClusters]float64
	perceptualKMeans(values, &centers, &bounds)
	want := [perceptualAQClusters]float64{1.5, 5.5, 9.5, 13.5, 17.5, 21.5, 25.5, 29.5}
	for i, w := range want {
	...
	for i := range perceptualAQClusters - 1 {
		wantBound := (want[i] + want[i+1]) / 2
		if math.Abs(bounds[i]-wantBound) > 1e-9 {
			t.Errorf("bound[%d] = %.6f, want %.6f", i, bounds[i], wantBound)
		}
	}
	if !math.IsInf(bounds[perceptualAQClusters-1], 1) {
		t.Errorf("last bound = %g, want +Inf", bounds[perceptualAQClusters-1])
	}
}
...
func TestVP9PerceptualMBWienerVarianceConstantBlock(t *testing.T) {
	...
	var mbVar [1]int64
	perceptualSetMBWienerVariance(src, stride, 16, 16, 1, 1, mbVar[:])
	if mbVar[0] != 0 {
...
func TestVP9PerceptualWienerVarSegmentMajorityVote(t *testing.T) {
	...
	bounds := [perceptualAQClusters]float64{1, 2, 3, 4, 5, 6, 7, math.Inf(1)}
	seg := perceptualWienerVarSegment(mbVar, 4, 4, 0, 0, &bounds)
	if seg != 0 {
