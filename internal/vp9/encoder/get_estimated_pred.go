...
// This file ports the get_estimated_pred orchestrator from libvpx v1.16.0
// vp9/encoder/vp9_encodeframe.c:5103-5198 — the
...
// GetEstimatedPredKeyFrame ports the keyframe branch of
...
func GetEstimatedPredKeyFrame(estPred []uint8) {
	for i := range 64 * 64 {
		estPred[i] = 128
	}
}

// GetEstimatedPredInterInput is the per-SB substrate the inter
...
// mi->ref_frame[0].
type GetEstimatedPredInterInput struct {
...
// 2 GOLDEN, 3 ALTREF.
type RefFrameSlot uint8

const (
	RefIntra  RefFrameSlot = 0
	RefLast   RefFrameSlot = 1
	RefGolden RefFrameSlot = 2
	RefAlt    RefFrameSlot = 3
)

// GetEstimatedPredInter ports the non-keyframe body of
...
func GetEstimatedPredSubBsize(miRow, miCol, miRows, miCols int) common.BlockSize {
...
// BuildEstimatedPredLuma64x64 ports the simplified
...
// GetEstimatedPred is the full orchestrator. Combines the keyframe
...
// (vector_var is exercised through IntProEstimate indirectly; the
