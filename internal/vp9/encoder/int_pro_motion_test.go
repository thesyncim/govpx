...
// Pinning tests for integer projection motion search. Reference values are
...
func TestVP9ClampMVClampsBothAxes(t *testing.T) {
	cases := []struct {
		...
		{"inside box", MV{Row: 5, Col: 7}, -10, 10, -10, 10, 5, 7},
		{"above max", MV{Row: 200, Col: 50}, -10, 10, -10, 100, 100, 10},
		{"below min", MV{Row: -500, Col: -42}, -10, 10, -50, 10, -50, -10},
	}
	for _, tc := range cases {
		mv := tc.in
		ClampMV(&mv, tc.minC, tc.maxC, tc.minR, tc.maxR)
...
func TestVP9FullpelMvLimitsClampAndContain(t *testing.T) {
	limits := &MvLimits{ColMin: -2, ColMax: 3, RowMin: -4, RowMax: 5}
...
func TestVP9SetFullpelMvSearchRangePinsLibvpxFormula(t *testing.T) {
	limits := &MvLimits{ColMin: -2000, ColMax: 2000, RowMin: -2000, RowMax: 2000}
	limits.SetFullpelSearchRange(MV{Row: 24, Col: -17})
...
func TestVP9SetSubpelMvSearchRangePinsLibvpxFormula(t *testing.T) {
	umv := &MvLimits{ColMin: -8, ColMax: 8, RowMin: -8, RowMax: 8}
	refMV := &MV{Row: 16, Col: 24}
	var out MvLimits
	SetSubpelMvSearchRange(&out, umv, refMV)
...
func TestVP9SetSubpelMvSearchRangePinsMvBoundClamp(t *testing.T) {
	umv := &MvLimits{ColMin: -100000, ColMax: 100000, RowMin: -100000, RowMax: 100000}
	refMV := &MV{Row: 0, Col: 0}
	var out MvLimits
	SetSubpelMvSearchRange(&out, umv, refMV)
	want := MvLimits{ColMin: -8184, ColMax: 8184, RowMin: -8184, RowMax: 8184}
	if out != want {
...
func TestVP9SetSubpelMvSearchRangeMvUppClampActive(t *testing.T) {
	umv := &MvLimits{ColMin: -200000, ColMax: 200000, RowMin: -200000, RowMax: 200000}
	refMV := &MV{Row: 0, Col: 18000}
	var out MvLimits
	SetSubpelMvSearchRange(&out, umv, refMV)
...
func TestVP9VectorMatchZeroOffset(t *testing.T) {
	...
	got := VectorMatch(ref, src, bwl)
	if got != 0 {
		t.Errorf("VectorMatch identity: got %d want 0", got)
	}
}
...
func TestVP9VectorMatchOffsetFoundByHierarchy(t *testing.T) {
	...
	got := VectorMatch(ref, src, bwl)
	if got != bw-bw/2 {
		t.Errorf("VectorMatch shift=bw: got %d want %d", got, bw-bw/2)
	}
}
...
func TestVP9IntProEstimateClampToMvLimits(t *testing.T) {
	...
	in := &IntProEstimateInput{
		...
		MvLimits: MvLimits{ColMin: -2, ColMax: 2, RowMin: -2, RowMax: 2},
	}
	_, mv := IntProEstimate(in)
