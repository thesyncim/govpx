...
// RCPickQAndBoundsTwoPassInputs aggregates the libvpx state the
// two-pass Q picker reads.
type RCPickQAndBoundsTwoPassInputs struct {
...
// RCPickQAndBoundsTwoPassResult is the (active_best, active_worst, q)
// tuple returned by libvpx vp9_rc_pick_q_and_bounds_two_pass.
type RCPickQAndBoundsTwoPassResult struct {
	ActiveBest  int
	ActiveWorst int
	Q           int
}
...
func RCPickQAndBoundsTwoPassWithRegulator(in RCPickQAndBoundsTwoPassInputs,
	...
	if in.IsIntraOnly {
		...
		if in.ThisKeyFrameForced {
			if in.LastKFGroupZeroMotionPct >= vp9StaticMotionThresh {
				...
				activeBest = qindex
				deltaQIndex := ComputeQDelta(in.BestQuality, in.WorstQuality, qindex, 125, 100)
				activeWorst = min(qindex+deltaQIndex, activeWorst)
			} else {
				qindex := in.LastBoostedQIndex
				deltaQIndex := ComputeQDelta(in.BestQuality, in.WorstQuality, qindex, 75, 100)
			...
			activeBest = KFActiveQualityWithBoost(activeWorst, keyFrameBoost)
			...
			activeBest += ComputeQDelta(in.BestQuality, in.WorstQuality,
		...
		activeBest = GFActiveQualityWithBoost(q, gfuBoost)
		...
		case 1:
			arfActiveBestQHL = GFHighMotionActiveQuality(q)
		case -1:
			arfActiveBestQHL = GFLowMotionActiveQuality(q)
		}
	...
	if in.IsIntraOnly || in.BoostFrame {
		...
		if in.RFLevel == RateFactorGFARFLow && in.LayerDepth > 1 {
	...
	return RCPickQAndBoundsTwoPassResult{
