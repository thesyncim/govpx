...
func ResidualSSE(residue []int16) uint64 {
	return transformBlockEnergyDispatch(residue)
}

func transformBlockEnergyScalar(coeffs []int16) uint64 {
	var sse uint64
	for _, diff := range coeffs {
		v := int64(diff)
		sse += uint64(v * v)
	}
	return sse
...
func TransformBlockError(coeffs, dqcoeffs []int16, txSize common.TxSize) uint64 {
	...
	err := transformBlockErrorDispatch(coeffs, dqcoeffs, n)
	if txSize != common.Tx32x32 {
		err >>= 2
	}
	return err
...
func transformBlockErrorScalar(coeffs, dqcoeffs []int16, n int) uint64 {
	var err uint64
	for i := range n {
