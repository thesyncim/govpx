...
//     model. The FindARFOrder closure for `calc_arf_boost` therefore
//     ignores the libvpx `mid`-frame stats_in advance
//     (vp9_firstpass.c:2180-2185). This divergence is currently
//     unreachable: libvpx's DefineGFGroupStructure (mirroring
...
//     absent from GFGroup. Promote when multi-ARF lands.
...
// GFGroup mirrors libvpx GF_GROUP field-for-field. Slice lengths use
// MaxStaticGFGroupLength+2 to match libvpx's MAX_STATIC_GF_GROUP_LENGTH+2.
//
...
const (
	...
	ARFAbsZoomThresh = 4.0
	// libvpx: vp9/encoder/vp9_firstpass.c:2561
	MaxGFBoost = 5400
	// libvpx: vp9/encoder/vp9_firstpass.c:2880
	LastALRActiveBestQAdjustFactor = 0.2
	// libvpx: vp9/encoder/vp9_ratectrl.h:31
	DefaultGFBoost = 2000
	// libvpx: vp9/encoder/vp9_encoder.c default two_pass_vbrbias.
	DefaultTwoPassVBRBiasPct = 50
	// libvpx: vp9/encoder/vp9_firstpass.c:262 ACT_AREA_CORRECTION.
	ActiveAreaCorrection = 0.5
)
...
const (
	...
	GFUpdate         uint8 = 2
	ARFUpdate        uint8 = 3
	OverlayUpdate    uint8 = 4
	MIDOverlayUpdate uint8 = 5
	UseBufFrame      uint8 = 6
)
...
type GFGroupInputs struct {
	...
	GFStartShowIdx int
	// Two-pass tuning parameters (ARFBoostParams).
...
// DefineGFGroup is a verbatim Go port of libvpx's define_gf_group. It
...
func DefineGFGroup(in GFGroupInputs) GFGroup {
	var gf GFGroup
	...
	activeGF := GetActiveGFIntervalRange(in, arfActiveOrKF)
	...
	gopIntraFactor := 1.0
	if in.MultiLayerARF {
		gopIntraFactor = 1.0 + 0.25*float64(GetARFLayers(in.MultiLayerARF, in.EnableAutoARF, activeGF.Max))
	}

	// libvpx: gop_coding_frames = get_gop_coding_frame_num(...)
	useAltRef := false
	endOfSequence := false
	gopCodingFrames := GetGOPCodingFrameNum(&useAltRef, in, &activeGF,
	...
	if useAltRef {
		...
		gfuBoost = ComputeARFBoost(in.Stats, arfShowIdx, fFrames, bFrames,
		...
		arfBoost := ComputeARFBoost(in.Stats, gldShowIdx, fFrames, bFrames,
			in.AvgFrameQIndexInter, in.BoostParams)
		gfuBoost = arfBoost
	...
	if !in.Lossless {
		...
		if in.FramesSinceKey >= in.FramesToKey {
			...
			gf.ARFActiveBestQAdjustF = LastALRActiveBestQAdjustFactor +
				(1.0-LastALRActiveBestQAdjustFactor)*
	...
	if in.PerceptualAQ && gfuBoost > MinARFGFBoost {
		gfuBoost = MinARFGFBoost
	}
	...
	for j := startIdx; j < gopCodingFrames; j++ {
		...
		// CalcNormFrameScoreConfig.
		gfGroupErr += calcNormFrameScoreFromInputs(fs, in)
	...
	gfGroupBits := CalculateTotalGFGroupBits(in, gf, arfActiveOrKF, gfGroupErr)
	...
	if baselineGFInterval > 1 && gopFrames > 0 {
		gf.ARNRStrengthAdjust = AdjustGroupARNRFilter(
	...
	gfARFBits := CalculateBoostBits(baselineGFInterval-1, gfuBoost, gfGroupBits)

	// libvpx: define_gf_group_structure(cpi) -> populates slot arrays.
	DefineGFGroupStructure(&gf, in)

	// libvpx: allocate_gf_group_bits(cpi, gf_group_bits, gf_arf_bits)
	AllocateGFGroupBits(&gf, in, gfGroupBits, gfARFBits, arfActiveOrKF)

	return gf
}

// GFRange mirrors libvpx's RANGE typedef.
//
...
// GetActiveGFIntervalRange ports libvpx get_active_gf_inverval_range.
//
// libvpx: vp9/encoder/vp9_firstpass.c:2701
func GetActiveGFIntervalRange(in GFGroupInputs, arfActiveOrKF bool) GFRange {
	arfBool := 0
	if arfActiveOrKF {
		arfBool = 1
	}
	intMaxQ := int(ConvertQIndexToQ(in.ActiveWorstQuality))
	var qTerm int
	if in.GFStartShowIdx == 0 {
		qTerm = intMaxQ / 32
	} else {
		qTerm = int(ConvertQIndexToQ(in.LastBoostedQIndex)) / 6
	}
	r := GFRange{}
...
// GetGOPCodingFrameNum ports libvpx get_gop_coding_frame_num.
//
// libvpx: vp9/encoder/vp9_firstpass.c:2587
func GetGOPCodingFrameNum(useAltRef *bool, in GFGroupInputs,
	activeGFInterval *GFRange, gopIntraFactor float64,
	endOfSequence *bool,
) int {
	loopDecayRate := 1.0
	...
	*useAltRef = true
	...
	for gopCodingFrames < in.StaticSceneMaxGFInterval &&
		...
		var nextNext *FirstPassFrameStats
		nnIdx := in.GFStartShowIdx + gopCodingFrames + 1
		if nnIdx >= 0 && nnIdx < len(in.Stats) {
			nnf := in.Stats[nnIdx]
			nextNext = &nnf
		}
		flashDetected := DetectFlashFromFrameStats(nextNext)

		// libvpx: accumulate_frame_motion_stats(next_frame, ...)
		AccumulateFrameMotionStats(nextFrame, &thisFrameMVInOut,
		...
		if !flashDetected {
			...
			if gopCodingFrames > in.MinGFInterval &&
				...
				stillInterval := 5
				if CheckTransitionToStill(in.Stats,
		...
		if gopCodingFrames >= activeGFInterval.Min &&
			...
			(mvRatioAccumulator > mvRatioAccumulatorThresh ||
				absMVInOutAccumulator > ARFAbsZoomThresh ||
...
// CheckTransitionToStill ports libvpx check_transition_to_still.
//
...
func CheckTransitionToStill(stats []FirstPassFrameStats, startIdx, stillInterval int, params ARFBoostParams) bool {
	...
	for i := range stillInterval {
		...
		dr := GetPredictionDecayRate(stats[idx], params.SRDiffFactor,
...
func GetARFLayers(multiLayerARF bool, maxLayers, codingFrameNum int) int {
	if !multiLayerARF {
		return 1
	}
	layers := 0
...
// CalcNormFrameScoreConfig ports libvpx calc_norm_frame_score
...
func CalcNormFrameScoreConfig(row FirstPassFrameStats,
	...
	bias := vbrBiasPct
	if bias <= 0 {
		bias = DefaultTwoPassVBRBiasPct
	}
	score := avErr * powSafe((err*weight)/avErr, float64(bias)/100.0)
	score *= powSafe(CalculateActiveArea(mbRows, row), ActiveAreaCorrection)
	...
	if maxSection <= 0 {
		maxSection = DefaultVBRMaxSectionPct
	}
...
// calcNormFrameScoreFromInputs threads the configured VBR bias / min /
// max from GFGroupInputs into CalcNormFrameScoreConfig. This is what
// DefineGFGroup uses when accumulating gf_group_err so the value matches
// libvpx under non-default oxcf->two_pass_vbr* settings.
func calcNormFrameScoreFromInputs(row FirstPassFrameStats,
	in GFGroupInputs,
) float64 {
	return CalcNormFrameScoreConfig(row, in.MeanModScore, in.AvErr,
...
// CalculateTotalGFGroupBits ports libvpx calculate_total_gf_group_bits.
//
// libvpx: vp9/encoder/vp9_firstpass.c:2057
func CalculateTotalGFGroupBits(in GFGroupInputs, gf GFGroup, arfActiveOrKF bool, gfGroupErr float64) int64 {
	...
	if in.KFGroupBits > 0 && in.KFGroupErrorLeft > 0 {
		...
		distFromNextKF := in.FramesToKey - gf.BaselineGFInterval
		if gf.SourceAltRefPending {
			distFromNextKF--
		}
...
// CalculateBoostBits ports libvpx calculate_boost_bits.
//
// libvpx: vp9/encoder/vp9_firstpass.c:2102
func CalculateBoostBits(frameCount, boost int, totalGroupBits int64) int {
	if boost == 0 || totalGroupBits <= 0 || frameCount < 0 {
		return 0
	}
	allocationChunks := frameCount*NormalBoost + boost
...
// FindARFOrder ports libvpx find_arf_order — recursive multi-ARF
...
func FindARFOrder(gf *GFGroup, indexCounter *int, depth, start, end int, calcArfBoost func(fFrames, bFrames int) int) {
	const minFrameInterval = 2
	mid := (start + end + 1) >> 1
	if end-start < minFrameInterval || depth > gf.AllowedMaxLayerDepth {
		for idx := start; idx <= end; idx++ {
			i := *indexCounter
			gf.UpdateType[i] = LFUpdate
			gf.ArfSrcOffset[i] = 0
			gf.FrameGOPIndex[i] = uint8(idx)
			gf.RFLevel[i] = RateFactorInterNormal
			gf.LayerDepth[i] = uint8(depth)
			gf.GFUBoost[i] = NormalBoost
			*indexCounter++
		}
	...
	i := *indexCounter
	gf.LayerDepth[i] = uint8(depth)
	gf.UpdateType[i] = ARFUpdate
	gf.ArfSrcOffset[i] = uint8(mid - start)
	gf.FrameGOPIndex[i] = uint8(mid)
	gf.RFLevel[i] = RateFactorGFARFLow
	boost := max(calcArfBoost(end-mid+1, mid-start)>>depth, MinARFGFBoost)
	gf.GFUBoost[i] = boost
	*indexCounter++

	FindARFOrder(gf, indexCounter, depth+1, start, mid-1, calcArfBoost)

	i = *indexCounter
	gf.UpdateType[i] = UseBufFrame
	gf.ArfSrcOffset[i] = 0
	gf.FrameGOPIndex[i] = uint8(mid)
	gf.RFLevel[i] = RateFactorInterNormal
	gf.LayerDepth[i] = uint8(depth)
	*indexCounter++
	...
	FindARFOrder(gf, indexCounter, depth+1, mid+1, end, calcArfBoost)
}

func setGFOverlayFrameType(gf *GFGroup, frameIdx int, sourceAltRefActive bool) {
	if sourceAltRefActive {
		gf.UpdateType[frameIdx] = OverlayUpdate
		gf.RFLevel[frameIdx] = RateFactorInterNormal
		gf.LayerDepth[frameIdx] = MaxARFLayers - 1
		gf.GFUBoost[frameIdx] = NormalBoost
	} else {
		gf.UpdateType[frameIdx] = GFUpdate
		gf.RFLevel[frameIdx] = RateFactorGFARFStd
		gf.LayerDepth[frameIdx] = 0
	}
}

// DefineGFGroupStructure ports libvpx define_gf_group_structure.
//
// libvpx: vp9/encoder/vp9_firstpass.c:2218
func DefineGFGroupStructure(gf *GFGroup, in GFGroupInputs) {
	frameIndex := 0
	keyFrame := in.IsKeyFrame
	layerDepth := 1
	gopFrames := gf.BaselineGFInterval
	if keyFrame || gf.SourceAltRefPending {
		gopFrames--
	}
	...
	if !keyFrame {
		setGFOverlayFrameType(gf, frameIndex, gf.SourceAltRefActive)
	}
	frameIndex++

	if gf.SourceAltRefPending {
		gf.UpdateType[frameIndex] = ARFUpdate
		gf.RFLevel[frameIndex] = RateFactorGFARFStd
	...
	calcArfBoost := func(fFrames, bFrames int) int {
		...
		return ComputeARFBoost(in.Stats, in.GFStartShowIdx+1, fFrames,
			bFrames, in.AvgFrameQIndexInter, in.BoostParams)
	}
	FindARFOrder(gf, &frameIndex, layerDepth, 1, gopFrames, calcArfBoost)

	setGFOverlayFrameType(gf, frameIndex, gf.SourceAltRefPending)
...
// AllocateGFGroupBits ports libvpx allocate_gf_group_bits.
//
// libvpx: vp9/encoder/vp9_firstpass.c:2391
func AllocateGFGroupBits(gf *GFGroup, in GFGroupInputs,
	...
	if corpus {
		avScore = DistributionAverageError(in.MeanModScore, in.AvErr)
	...
	for i := range normalFrames {
		...
		if corpus {
			...
			thisFrameScore := CalcNormFrameScoreCorpus(in.Stats[showIdx], avScore,
...
// DistributionAverageError mirrors libvpx get_distribution_av_err for the
...
func DistributionAverageError(meanModScore, avErr float64) float64 {
	if meanModScore <= 0 {
		if avErr > 0 {
			return avErr
		}
...
// CalcNormFrameScoreCorpus ports libvpx's calc_norm_frame_score helper
// used by the corpus-VBR branch of allocate_gf_group_bits. Unlike the
// older root-package score helpers (which hardcoded the libvpx default
...
func CalcNormFrameScoreCorpus(row FirstPassFrameStats, avErr, meanModScore float64,
	...
	if active < MinActiveAreaFP {
		active = MinActiveAreaFP
	} else if active > MaxActiveAreaFP {
		active = MaxActiveAreaFP
	}
	modifiedScore *= math.Pow(active, ActiveAreaCorrection)
...
// CalculateGroupScore mirrors libvpx calculate_group_score, summing the
...
func CalculateGroupScore(in GFGroupInputs, avScore float64, frameCount int) float64 {
	if frameCount <= 0 {
	...
	for i := range frameCount {
		...
		scoreTotal += CalcNormFrameScoreCorpus(in.Stats[showIdx], avScore,
			in.MeanModScore, in.TwoPassVBRBiasPct,
			in.TwoPassVBRMinSection, in.TwoPassVBRMaxSection, in.MBRows)
	}
	if scoreTotal <= 0 {
...
// AdjustGroupARNRFilter ports libvpx adjust_group_arnr_filter.
//
// libvpx: vp9/encoder/vp9_firstpass.c:2541
func AdjustGroupARNRFilter(sectionNoise, sectionInter, sectionMotion float64) int {
