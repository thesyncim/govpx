...
//go:noescape
func convolveAvgSSE2(src *byte, srcStride int, dst *byte, dstStride int, w, h int)
...
func vpxConvolveAvg(src []byte, srcStride int, dst []byte, dstStride, w, h, srcOffset int) {
	...
	convolveAvgSSE2(unsafe.SliceData(src[srcOffset:]), srcStride, unsafe.SliceData(dst), dstStride, w, h)
}
