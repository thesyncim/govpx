...
func BenchmarkCoefBlockTokenRateWithTableTrusted(b *testing.B) {
	probs := vp8tables.DefaultCoefProbs
	var costs CoefficientTokenCostTable
	FillCoefficientTokenCostTable(&probs, &costs)
	var qcoeff [16]int16
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		sink += CoefficientBlockTokenRateWithTableTrusted(&costs, 0, 0, 1, &qcoeff, 11)
		sink += CoefficientBlockTokenRateWithTableTrusted(&costs, 3, 0, 0, &qcoeff, 11)
		sink += CoefficientBlockTokenRateWithTableTrusted(&costs, 2, 0, 0, &qcoeff, 8)
		sink += CoefficientBlockTokenRateWithTableTrusted(&costs, 1, 0, 0, &qcoeff, 11)
	}
	_ = sink
}
...
func BenchmarkCoefBlockTokenRateZeroWithTableTrusted(b *testing.B) {
	probs := vp8tables.DefaultCoefProbs
	var costs CoefficientTokenCostTable
	FillCoefficientTokenCostTable(&probs, &costs)
	var qcoeff [16]int16
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		sink += CoefficientBlockTokenRateWithTableTrusted(&costs, 0, 0, 1, &qcoeff, 1)
		sink += CoefficientBlockTokenRateWithTableTrusted(&costs, 3, 0, 0, &qcoeff, 0)
	}
	_ = sink
}
