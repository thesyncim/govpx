...
// TestTask241WriterEmitsLibvpxTupleOrder pins the (block, band, ctx, node)
...
func TestTask241WriterEmitsLibvpxTupleOrder(t *testing.T) {
	const (
		B = tables.BlockTypes
		J = tables.CoefBands
...
// TestTask241BuilderDefaultPathStrictSavingsGate pins the libvpx `s > 0`
...
func TestTask241BuilderDefaultPathStrictSavingsGate(t *testing.T) {
...
// TestTask241WriterNoForceEmitOnDefaultPath pins that the default-path
...
// force-emit.
func TestTask241WriterNoForceEmitOnDefaultPath(t *testing.T) {
	var counts coefficientTokenCounts
	base := tables.DefaultCoefProbs
	_, updates, err := coefficientProbabilityUpdatesFromTokenCounts(&base, &counts)
	if err != nil {
