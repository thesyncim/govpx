...
// gatherV8x8PairAMD64SSE2 ABI ($0-32):
//   tmp+0(FP)     *[128]byte
//   u+8(FP)       *byte
//   v+16(FP)      *byte
//   stride+24(FP) int
TEXT ·gatherV8x8PairAMD64SSE2(SB), NOSPLIT, $0-32
	MOVQ	tmp+0(FP), DI
	MOVQ	u+8(FP), AX
	MOVQ	v+16(FP), CX
	MOVQ	stride+24(FP), BX

	// U plane rows 0..7 -> tmp row low qwords.
	MOVQ	AX, R8
	MOVQ	(R8), X0
	ADDQ	BX, R8
	MOVQ	(R8), X1
	...
	// V plane rows 0..7 -> tmp row high qwords.
	MOVQ	CX, R8
	MOVQ	(R8), X0
	ADDQ	BX, R8
	MOVQ	(R8), X1
...
// scatterV8x8PairAMD64SSE2 ABI ($0-32):
//   u+0(FP)       *byte
//   v+8(FP)       *byte
//   stride+16(FP) int
//   tmp+24(FP)    *[128]byte
TEXT ·scatterV8x8PairAMD64SSE2(SB), NOSPLIT, $0-32
	MOVQ	u+0(FP), AX
	MOVQ	v+8(FP), CX
	MOVQ	stride+16(FP), BX
	MOVQ	tmp+24(FP), DI

	// tmp low qwords -> U plane rows 0..7.
	MOVQ	0(DI), X0
	MOVQ	16(DI), X1
	MOVQ	32(DI), X2
	MOVQ	48(DI), X3
	...
	// tmp high qwords -> V plane rows 0..7.
	MOVQ	8(DI), X0
	MOVQ	24(DI), X1
	MOVQ	40(DI), X2
	MOVQ	56(DI), X3
