...
// hand-written SSE2. The rectangular 16x8 and 8x16 forms use direct
// kernels so the overlapping horizontal rows are computed once.
//
...
//go:noescape
func sixTapPredict16x8SSE2(dst *byte, dstStride int, src *byte, srcStride int,
...
func sixTapPredict16x8Maybe(src []byte, srcStride int, xoffset int, yoffset int,
	...
	hFilter := &tables.SubPelFilters[xoffset]
	vFilter := &tables.SubPelFilters[yoffset]
	sixTapPredict16x8SSE2(unsafe.SliceData(dst), dstStride, unsafe.SliceData(src), srcStride, hFilter, vFilter, &tmp)
	return true
}
