...
//
// Filter rows are *[6]int16 (signed; values in [-16, 123] plus the
// identity 128). Arithmetic follows libvpx's NEON kernels exactly:
// each |tap| is broadcast as uint8 lanes and multiplied with UMULL/
// UMLAL/UMLSL into a modular uint16 accumulator (taps 1 and 4 are
// non-positive for every VP8 subpel filter row, taps 0/2/5
// non-negative), the center tap 3 accumulates separately, and the two
// halves combine with SQADD (int16 saturating) before SQRSHRUN rounds
// and saturates to uint8. Partial wraparound in the modular u16 sum
// cancels; the only nonlinear steps (SQADD + SQRSHRUN) are the same
// ones libvpx performs, so the output is bit-identical to the scalar
// reference. Mnemonics missing from Go's arm64 assembler are emitted
// as raw WORD directives.
//
...
//   V10,V11 uint16x8 accumulators (lo/hi lanes)
//   V12,V13 center-tap uint16x8 products (lo/hi lanes)
...
// ABS16 loads the int16 tap at off(Rf) sign-extended and leaves |tap|
// in R7 for the following dup-to-uint8 WORD.
#define ABS16(off, Rf) \
	MOVH	off(Rf), R7 \
	ASR	$63, R7, R10 \
	EOR	R10, R7, R7 \
...
horiz_loop:
	...
	// Tap 0: V_tap = V0 (bytes [0..15]); tap 0 is non-negative.
	WORD	$0x2e30c00a	// umull  v10.8h, v0.8b,  v16.8b
	WORD	$0x6e30c00b	// umull2 v11.8h, v0.16b, v16.16b

	// Tap 1: bytes [1..16] (Plan9 order: VEXT $1, V1, V0); non-positive.
	...
	// Tap 3 (center): bytes [3..18]; separate accumulator for the
	// final saturating combine, exactly as libvpx.
	...
	// === Vertical pass: 16 rows, rolling 6-row window ===
