...
// sixTapPredict16x8SSE2 ABI ($0-56):
//   dst+0(FP)        *byte
//   dstStride+8(FP)  int
//   src+16(FP)       *byte
//   srcStride+24(FP) int
...
//
// Same 16-column kernel as sixTapPredict16x16SSE2, with H+5=13
// horizontal rows and H=8 vertical rows. This replaces the older
// two-8x8 composition and avoids recomputing the five overlapping
// horizontal rows.

TEXT ·sixTapPredict16x8SSE2(SB), NOSPLIT, $32-56
	MOVQ	dst+0(FP), DI
	MOVQ	dstStride+8(FP), BX
	MOVQ	src+16(FP), SI
	MOVQ	srcStride+24(FP), CX
	...
	// === Horizontal pass: 13 rows of 16 cols ===
	MOVQ	$13, R13
	MOVQ	R10, R14
horiz16x8_loop:
	...
	// === Vertical pass: 8 rows of 16 cols ===
	MOVQ	$8, R13
	MOVQ	R10, R14
vert16x8_loop:
