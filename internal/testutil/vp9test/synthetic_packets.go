...
// SkipZeroKeyframe returns a keyframe fixture whose only block is coded
// skip=0. If writeResidue is false, the packet stops before coefficient data
// and is intentionally incomplete.
func SkipZeroKeyframe(t testing.TB, width, height int, writeResidue bool) []byte {
	t.Helper()
	return SkipResidueKeyframe(t, width, height, writeResidue, 0)
}

// SkipResidueKeyframe is like SkipZeroKeyframe, but writes dcCoeff into the
// first luma transform block when residue is enabled.
func SkipResidueKeyframe(t testing.TB, width, height int,
