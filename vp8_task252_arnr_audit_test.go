...
// vp8_arnr_temporal_filter_audit_test.go (apply, window, fixed_divide)
...
// vp8encSourceImageForTask252 constructs the minimal vp8enc.SourceImage
...
// the helper reusable.
func vp8encSourceImageForTask252(width, height int, y, u, v []byte) vp8enc.SourceImage {
...
// libvpxIterateReferenceTask252 is a byte-for-byte transcription of
...
func libvpxIterateReferenceTask252(t *testing.T, dst *arnrFrameView, refs []arnrFrameView, centerIdx int, strength int) {
	t.Helper()
...
// TestVP8Task252ARNRIterateByteExact pins the VP8 ARNR iterate plumbing
// against a libvpx-verbatim transcription of
// vp8/encoder/temporal_filter.c:188-346.
func TestVP8Task252ARNRIterateByteExact(t *testing.T) {
	const (
		width  = 48
		height = 32
	)
	...
	for _, strength := range []int{1, 3, 5} {
		t.Run("strength_"+task252Itoa(strength), func(t *testing.T) {
			...
			copyARNRTask252Plane(e.arnrScratch.Img.Y, e.arnrScratch.Img.YStride, cy, width, width, height)
			copyARNRTask252Plane(e.arnrScratch.Img.U, e.arnrScratch.Img.UStride, cu, (width+1)>>1, (width+1)>>1, (height+1)>>1)
			copyARNRTask252Plane(e.arnrScratch.Img.V, e.arnrScratch.Img.VStride, cv, (width+1)>>1, (width+1)>>1, (height+1)>>1)

			centerSrc := vp8encSourceImageForTask252(width, height, cy, cu, cv)
			...
			libvpxIterateReferenceTask252(t, &refDst, refs, centerIdx, strength)

			compareTask252Bytes(t, "Y", e.arnrScratch.Img.Y, e.arnrScratch.Img.YStride, refDst.y, refDst.yStride, width, height)
			uvW := (width + 1) >> 1
			uvH := (height + 1) >> 1
			compareTask252Bytes(t, "U", e.arnrScratch.Img.U, e.arnrScratch.Img.UStride, refDst.u, refDst.uStride, uvW, uvH)
			compareTask252Bytes(t, "V", e.arnrScratch.Img.V, e.arnrScratch.Img.VStride, refDst.v, refDst.vStride, uvW, uvH)
		})
	}
}

func copyARNRTask252Plane(dst []byte, dstStride int, src []byte, srcStride int, w int, h int) {
	for y := range h {
		copy(dst[y*dstStride:y*dstStride+w], src[y*srcStride:y*srcStride+w])
	}
}

func compareTask252Bytes(t *testing.T, plane string, got []byte, gotStride int, want []byte, wantStride int, w int, h int) {
	t.Helper()
	for y := range h {
		for x := range w {
			g := got[y*gotStride+x]
