...
// vp8Task253FirstByteDiff returns the byte offset of the first
...
func vp8Task253FirstByteDiff(a, b []byte) int {
	n := min(len(b), len(a))
	for i := range n {
		if a[i] != b[i] {
			return i
...
// vp8_task253_multires_svc_parity_test.go exercises the previously
...
type vp8Task253MultiResLayer struct {
	Name        string
	Width       int
	Height      int
	BitrateKbps int
...
// vp8Task253DownsampleI420 downsamples an I420 Image to the target
...
func vp8Task253DownsampleI420(src Image, dstW, dstH int) Image {
...
// vp8Task253MultiResStreams runs an N-rendition simulcast cluster and
...
func vp8Task253MultiResStreams(
	t *testing.T,
	parentW, parentH, fps, frames int,
	deadline Deadline,
	cpuUsed int,
	layers []vp8Task253MultiResLayer,
) [][][]byte {
	t.Helper()
	...
	for f := range frames {
		...
		for i, l := range layers {
			src := vp8Task253DownsampleI420(parent, l.Width, l.Height)
...
// vp8Task253SVCStreams runs the temporal SVC pattern through govpx and
...
// TestVP8Task253MultiResSVCParityBaseline plants the byte-parity
...
func TestVP8Task253MultiResSVCParityBaseline(t *testing.T) {
	t.Run("multires-2layer-simulcast", func(t *testing.T) {
		...
		const frames = 4
		runA := vp8Task253MultiResStreams(t, 640, 360, 30, frames, DeadlineRealtime, -3, layers)
		runB := vp8Task253MultiResStreams(t, 640, 360, 30, frames, DeadlineRealtime, -3, layers)
			...
			for f := 0; f < len(runA[i]); f++ {
				...
				if ha != hb {
					t.Errorf("layer %d (%q) frame %d non-deterministic: len_a=%d len_b=%d first_diff=%d",
						...
						vp8Task253FirstByteDiff(runA[i][f], runB[i][f]))
				}
			}
		}
	})
	...
	t.Run("svc-2layer-2temporal", func(t *testing.T) {
		...
		runA := vp8Task253SVCStreams(t, w, h, fps, frames, -3, TemporalLayeringTwoLayers, 2, bitrates)
		runB := vp8Task253SVCStreams(t, w, h, fps, frames, -3, TemporalLayeringTwoLayers, 2, bitrates)
			...
			for f := 0; f < len(runA[layer]); f++ {
				...
				if ha != hb {
					t.Errorf("svc layer %d frame %d non-deterministic: len_a=%d len_b=%d first_diff=%d",
						...
						vp8Task253FirstByteDiff(runA[layer][f], runB[layer][f]))
				}
			}
		}
	})
	...
	t.Run("svc-2temporal-cpu-3", func(t *testing.T) {
		...
		bitrates := [5]int{420, 700, 0, 0, 0}
		runA := vp8Task253SVCStreams(t, w, h, fps, frames, -3, TemporalLayeringTwoLayersThreeFrame, 2, bitrates)
		runB := vp8Task253SVCStreams(t, w, h, fps, frames, -3, TemporalLayeringTwoLayersThreeFrame, 2, bitrates)
		for layer := range 2 {
			if len(runA[layer]) != len(runB[layer]) {
				t.Fatalf("layer %d frame count drift a=%d b=%d", layer, len(runA[layer]), len(runB[layer]))
			}
...
// TestVP8Task253TemporalLayerIDOverride pins libvpx's runtime layer-id
...
// exercise.
func TestVP8Task253TemporalLayerIDOverride(t *testing.T) {
...
// TestVP8Task253MultiResIndependentEncoders pins the rule from
...
func TestVP8Task253MultiResIndependentEncoders(t *testing.T) {
	layers := []vp8Task253MultiResLayer{
		{Name: "low", Width: 320, Height: 180, BitrateKbps: 200},
		{Name: "high", Width: 640, Height: 360, BitrateKbps: 800},
	}
	...
	for i, l := range layers {
		single := []vp8Task253MultiResLayer{l}
		runs := vp8Task253MultiResStreams(t, 640, 360, 30, frames, DeadlineRealtime, -3, single)
		refStreams[i] = runs[0]
	}
	// Interleaved: all encoders advancing frame-by-frame.
	interleaved := vp8Task253MultiResStreams(t, 640, 360, 30, frames, DeadlineRealtime, -3, layers)
	for i, l := range layers {
		...
		for f := 0; f < len(refStreams[i]); f++ {
			...
			if gh != rh {
				t.Errorf("layer %d (%q) frame %d schedule-dependent: interleaved_len=%d ref_len=%d first_diff=%d",
					...
					vp8Task253FirstByteDiff(interleaved[i][f], refStreams[i][f]))
			}
		}
	}
}
