...
func TestOracleEncoderStreamByteParitySegmentation(t *testing.T) {
	...
	cases := []struct {
		...
		// screen-content2-panning-256x144 at cpu-3: govpx encodes
		// all 16 frames while libvpx drops one inter frame. This
		// surfaces a CBR drop-gate divergence at the larger frame
		// + sc=2 cross-product; pin as a known gap until the
		// drop decision is brought into parity.
		{name: "screen-content2-panning-256x144-realtime-cpu-3", deadline: DeadlineRealtime, cpuUsed: -3, fx: panning256x144, screenContentMode: 2, limit: -1, extraArgs: []string{"--screen-content-mode=2"}},
