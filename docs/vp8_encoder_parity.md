...
- Reconstruction byte-identity (measured 2026-05-08 by capturing both
  ...
  - 128x128 panning realtime CBR CpuUsed 8 (round 2, after the recode-loop
    parity gate that closed the Q drift): pre-loop-filter inter MB output
    is byte-identical for all 64 MBs of frame 1 (predictor + post-residual
    Y/U/V dumps via `oracle_chroma_subpel_predictor_diag_test.go`). The
    cascading divergence at frame 2+ is **driven by a loop-filter level
    fast-picker mismatch** (govpx LF=11 vs libvpx LF=5 for frame 1 q=16),
    not chroma sixtap math. Frame 2's LAST reference inherits the
    different LF strength and feeds different chroma sub-pel inputs;
    frame 2's MV / mode / ref decisions still match libvpx exactly, so
    the predictor diff is purely a downstream consequence.
...
| 128x128 / 96x96 / 160x96 panning realtime cbr cpu-used=8 | q-bounds 4..56, 4 frames | keyframe y/u/v Adler32 + Q byte-identical on all three sizes; inter-frame y/u/v Adler32 differ on every inter frame; per-frame size delta is +0.07..+10.1% with the largest gap on 96x96 frame 1 (-10.1%) and 160x96 frame 1/2 (-8.2 / +8.8%, with a 1-step Q drift on 160x96 frame 2). Decoded-output diff stays within max\|delta\|=4 (Y) / 3 (U) / 1 (V) with mean\|delta\|<0.04 (PSNR vs libvpx > 60 dB) | **Round-2 localization (2026-05-08)**: per-MB predictor + reconstructed dumps confirm the inter prediction and post-residual MB content are byte-identical for every MB of frame 1 on the 128x128 panning fixture (64 MBs, Y/U/V, all rows when `GOVPX_DEBUG_ALL_ROWS=1`). The frame 2+ predictor diffs cascade from a **loop-filter level fast-picker divergence** (govpx LF=11 vs libvpx LF=5 for frame 1 at q=16, identical clamped seed). The original "chroma sixtap rounding" hypothesis is falsified by matched predictor + reconstructed dumps. Tracked by [`TestOracleChromaSubpelScoreboard`](../oracle_chroma_subpel_scoreboard_test.go) with a per-fixture baseline at [`testdata/chroma_subpel_scoreboard_baseline.json`](../testdata/chroma_subpel_scoreboard_baseline.json). Diagnostic harness lives in [`oracle_chroma_subpel_predictor_diag_test.go`](../oracle_chroma_subpel_predictor_diag_test.go) (gate `GOVPX_DEBUG=1`, optional `GOVPX_DEBUG_ALL_ROWS=1`). **R9-5 (2026-05-08) refinement**: with the libvpx full-picker oracle now emitting a `pre 0` row (full-frame Y SSE between source and unfiltered recon, added in [`internal/coracle/build_vpxenc_oracle.sh`](../internal/coracle/build_vpxenc_oracle.sh)), the per-trial-level diff harness ([`oracle_lf_trial_diag_test.go`](../oracle_lf_trial_diag_test.go)) shows the LF-picker SSE region/accumulator are byte-identical to libvpx (`vp8_calc_ss_err` matches `loopFilterLumaSSE(partial=false)` to the bit on frame 0; `calc_partial_ssl_err` reads the same MB grid as govpx's partial path). The remaining per-trial-level Y SSE deltas (-251 to +2543 across frames 1..3) are an **upstream pre-LF reconstruction divergence**: the unfiltered analysis buffer differs by SSE 47..2533 *before any LF apply*, so the picker is feeding correctly-by-libvpx-spec inputs through a correctly-by-libvpx-spec SSE and arriving at different chosen levels. Frame 1's picker-flavor mismatch (govpx full vs libvpx fast) is also non-deterministic: `vp8_auto_select_speed` evolves Speed based on per-frame encode timing, so the two encoders' Speed values can flip on the boundary that drives the `auto_filter` 0/1 selection. The 64x64 LF header scoreboard stays at 100% level/sharp/deltas/enabled/update across realtime cpu-used 0/8/15 + good-quality cpu-used 3 vbr fixtures; the per-trial gap surfaces only on 128x128 panning where the upstream recon drift accumulates. |
...
| 2026-05-08 | parity-close-r2-c-chroma-subpel | per-MB predictor + post-residual + LAST-reference dumps across all 64 MBs of inter frame 1 (128x128 panning realtime CBR cpu-used=8) via the new `oracle_chroma_subpel_predictor_diag_test.go` harness | localized | Predictor and reconstructed Y/U/V byte-identical on every MB of frame 1 (192 plane rows). LAST reference for frame 2 differs (94 U bytes, 48 V bytes, 31 Y bytes inside the 16+CodedW window) because govpx picked `loop_filter_level=11` vs libvpx's 5 for frame 1 (same q=16, same clamped seed, same partial-frame algorithm). Frame 2 predictor + reconstructed diffs cascade from the LAST-reference drift, not from the chroma sixtap math. Harness lives in `oracle_chroma_subpel_predictor_diag_test.go`; the libvpx oracle exposes the capture via `GOVPX_ORACLE_PREDICTOR_DUMP` / `GOVPX_ORACLE_PREDICTOR_DUMP_ALL_ROWS` env vars and the govpx side mirrors them through `EncoderOptions.OracleTracePredictorDump` / `OracleTracePredictorDumpAllRows`. |
