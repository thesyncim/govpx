...
- Root Go files: 747.
- Root test files: 583.
- Internal Go files: 754.
- Root VP8 files: 82 implementation files and 320 test files.
- Root VP9 files: 73 implementation/facade files and 253 test files.
- Internal package files:
  ...
  - `internal/testutil`: 54 Go files.
- Test-name clusters:
  - Non-internal tests: 602.
  - Internal tests: 322.
  - Test files with `oracle` in the name: 183.
  - Test files with `parity` in the name: 91.
...
- Keep true root facade/API tests in root: RTP facade, active maps, worker
  ...
  perceptual-AQ header behavior, rate-control-bound validation, synthetic
  multi-tile keyframe decode, and public encoder-to-decoder roundtrip facade
  coverage now runs from `package govpx_test`.
