...
- Root VP9 files: 73 implementation/facade files and 245 test files.
...
- Keep true root facade/API tests in root: RTP facade, active maps, worker
  ...
  frame-info, DecodeInto, corpus, stream-info, and public encoder-to-decoder
  roundtrip facade coverage now runs from `package govpx_test`.
