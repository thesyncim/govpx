...
- Root Go files: 742.
- Root test files: 578.
- Internal Go files: 752.
- Root VP8 files: 82 implementation files and 320 test files.
- Root VP9 files: 73 implementation/facade files and 248 test files.
...
- Keep true root facade/API tests in root: RTP facade, active maps, worker
  ...
  loop-filter-opt validation, public superframe packing/decoding, encoder and
  ...
  perceptual-AQ header behavior, rate-control-bound validation, and public
  encoder-to-decoder roundtrip facade coverage now runs from `package govpx_test`.
