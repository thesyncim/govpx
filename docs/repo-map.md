...
- Root Go files: 737.
- Root test files: 573.
- Internal Go files: 751.
- Root VP8 files: 82 implementation files and 319 test files.
- Root VP9 files: 73 implementation/facade files and 243 test files.
- Internal package files:
  ...
  - `internal/testutil`: 52 Go files.
- Test-name clusters:
  - Non-internal tests: 597.
  - Internal tests: 321.
  - Files with `oracle` in the name: 191.
  - Files with `parity` in the name: 92.
...
- Keep true root facade/API tests in root: RTP facade, active maps, worker
  ...
  byte-alignment, header-rejection, postprocess/error-concealment, decryptor,
  external-frame-buffer, show-existing, frame-info, DecodeInto, corpus, and
  public encoder-to-decoder roundtrip facade coverage now runs from
  `package govpx_test`.
