...
Last refreshed: 2026-05-21 from `main`.

## Current Counts

- Root Go files: 516.
- Root test files: 361.
- Internal Go files: 684.
...
- Test-name clusters:
  ...
  - Files with `bench` in the path: 40.
...
helpers, geometry/validation, arithmetic helpers, and small shared value
objects. Do not move codec syntax, mode decisions, probability models, or
reference semantics into `internal/vpx`.
...
helpers. Oracle-tagged root tests may use coracle; default-build tests and
...
| Root tests | 361 top-level root tests remain; many are codec implementation and parity tests; shared VP9 YCbCr/I420/header helpers now live in `internal/testutil/vp9test`; VP9 external corpus selection and minimum rules now live in `internal/testutil/vp9corpus`; VP8 and VP9 RTP fuzzers now live beside the internal RTP packages. | Public facade tests remain in root; implementation tests move beside internal packages; reusable helpers move to `internal/testutil` or `internal/coracle`. |
