...
| P0 | FDCT16x16 | arm64 | `internal/vp9/encoder/transform_quant_dispatch_arm64.go`, new `fdct16x16_arm64.s` | `vpx_dsp/arm/fdct16x16_neon.c::vpx_fdct16x16_neon` | scalar fallback; dispatch-vs-scalar benchmark exists | `go test ./internal/vp9/encoder -run 'TestForwardDCT16x16|TestForwardHT16x16' -count=1`; `go test ./internal/vp9/encoder -run '^$' -bench '^BenchmarkForwardDCT16x16' -benchmem -benchtime=500ms -count=5`; add direct SIMD-vs-scalar tests before routing |
