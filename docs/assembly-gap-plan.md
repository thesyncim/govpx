...
| P0 | FDCT4x4/8x8/WHT4x4 | amd64 | `internal/vp9/encoder/transform_quant_dispatch_amd64.go`, `internal/vp9/encoder/fdct4x4_amd64.s`, `internal/vp9/encoder/fdct8x8_amd64.s`, `internal/vp9/encoder/fwht_amd64.s` | `vpx_dsp/x86/fwd_txfm_impl_sse2.h::vpx_fdct8x8_sse2`, `vp9/encoder/x86/vp9_dct_sse2.c` | FDCT4x4, FDCT8x8, and WHT4x4 use SSE2 with scalar fallback; FDCT16x16 remains scalar on amd64 | `GOARCH=amd64 go test ./internal/vp9/encoder -run 'TestForward(DCT4x4|DCT8x8|WHT4x4)|TestForwardDCT8x8SSE2' -count=1`; direct SIMD-vs-scalar benches |
...
| P1 | Compound-average convolve | arm64/amd64 | `internal/vp9/dsp/convolve_*.go` | `vpx_dsp/arm/vpx_convolve8_neon.c`, `vpx_dsp/x86/vpx_subpixel_8t_intrin_*` | plain 8-tap horiz/vert/full SIMD exists; amd64 `VpxConvolveAvg` now uses exact SSE2 `PAVGB` for 8+ wide compound blends; avg horiz/vert and arm64 avg remain scalar | `GOVPX_WITH_ORACLE=1 GOARCH=amd64 go test -tags govpx_oracle_trace ./internal/vp9/dsp -run '^TestDSPMatchesLibvpx$' -count=1`; `GOARCH=amd64 go test ./internal/vp9/dsp -run '^$' -bench '^BenchmarkVP9Convolve(Avg|8Avg)' -benchmem -benchtime=500ms -count=5` |
| P1 | DC-only IDCT add | amd64 | `internal/vp9/dsp/idct_dispatch_amd64.go`, `idct_amd64.s` | `vpx_dsp/x86/inv_txfm_sse2.c::vpx_idct*_1_add_sse2` | `Idct{4,8,16,32}x*_1Add` now compute the source-shaped scalar DC prelude and route valid windows through SSE2 broadcast-add kernels; purego and invalid windows fall back scalar | `GOARCH=amd64 go test ./internal/vp9/dsp -run '^(TestVP9IdctDcSimd(Agreement|EdgeCases|StrideLargerThanWidth)|TestIdct4x4DcOnlyMatchesFullPath|TestIdct8x8DcOnlyMatchesFullPath|TestVP9IdctFullSimd(Agreement|SingleCoefficient))$' -count=1`; `GOARCH=amd64 go test ./internal/vp9/dsp -run '^$' -bench 'BenchmarkVP9Idct(4x4_1Add|8x8_1Add|16x16_1Add|32x32_1Add)$' -benchmem -count=5` |
...
| P0 | Six-tap split predictors | amd64 | `internal/vp8/dsp/subpixel_amd64.go`, `subpixel_amd64.s` | `vp8/common/x86/subpixel_sse2.asm`, `vp8/common/x86/vp8_asm_stubs.c` | 16x16/16x8/8x16/8x8/8x4/4x4 SSE2 exist; future work is profile-gated AVX2 or pair-batch variants | `go test ./internal/vp8/dsp -run 'TestSixTapPredict.*|FuzzVP8DSPSubpixel' -count=1`; `go test ./internal/vp8/dsp -run '^$' -bench 'BenchmarkSixTapPredict(16x8|8x16)$' -benchmem -benchtime=500ms -count=5` on amd64 |
| P0 | Fused subpel variance16x16 | amd64 | `internal/vp8/dsp/variance_subpel16_fused_amd64.go`, `variance_subpel_amd64.go` | `vpx_dsp/x86/subpel_variance_sse2.asm`, `vpx_dsp/x86/variance_sse2.c` | amd64 routes 16x16 horizontal/vertical/bilinear through staged SSE2 first-pass, second-pass, and variance kernels; true fused asm remains future profile-gated work | `go test ./internal/vp8/dsp -run 'TestSubpelVariance.*SIMD|TestVarianceBlocks' -count=1`; `go test ./internal/vp8/dsp -run '^$' -bench 'BenchmarkSubpelVariance16x16(Dispatch|PtrFast|HorizontalOnly|VerticalOnly)' -benchmem -benchtime=500ms -count=5` on amd64 |
...
1. Finish VP9 amd64 FDCT8x8 dispatch. FDCT4x4, FP quantize, and WHT4x4 are
...
5. VP8 true fused 16x16 subpel variance asm, but only after fresh profiling
   shows the staged amd64 SSE2 route still leaves meaningful time on the table.
