...
- `EncodeInto` can emit source-dependent DCPred keyframes, LAST/ZEROMV
  residual interframes, and simple full-pixel NEWMV interframes against the
  reconstructed last reference with last/golden/altref refresh control, but full
  prediction mode analysis, broader motion search, and rate-control feedback are
  not complete yet.
