...
| IVF/test vectors | IVF parser and oracle checksum parser scaffolded; normal-test VP8/IVF smoke vectors, including libvpx-authored reference, token-partition, profile, loop-filter sharpness, and error-resilient streams plus NEWMV, subpixel NEWMV, luma/chroma intra-mode keyframe, and intra-macroblock interframe coverage, checksummed against libvpx v1.16.0; opt-in encoder oracle coverage includes B_PRED keyframes and intra B_PRED interframes; opt-in generated libvpx corpus covers profile 0/1/2/3, one/two/four/eight token partitions, active eight-partition row cycling, error-resilient cyclic-refresh segmentation, sharpness, and static-threshold streams with feature assertions; opt-in external VP8 IVF conformance via `LIBGOPX_TEST_DATA_PATH` with required/minimum-count controls for CI corpus runs |
...
- `EncodeInto` can emit source-dependent whole-block luma/chroma intra keyframes,
  ...
  libvpx rate-control heuristic parity are not complete yet.
