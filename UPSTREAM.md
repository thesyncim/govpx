...
| Subsystem | Status |
| --- | --- |
| Public API (VP9Decoder / VP9Encoder) | scaffolded. VP9 decoder construction / option validation, `Decode`, `DecodeWithPTS`, `DecodeInto`, `LastFrameInfo`, `Reset`, and `NextFrame` are wired for 8-bit 4:2:0 intra frames: mode-info, detokenize, intra prediction, inverse transform/add, reference refresh, show-existing-frame display, PTS metadata, caller-owned output planes, configured resolution-change rejection, VP9's four frame-context slots / `refresh_frame_context` gate, single-reference zero-MV inter frames across skipped and non-skipped residual blocks, scaled-reference zero-MV / NEWMV / NEARESTMV / NEARMV inter frames, compound zero-MV inter frames, inter-frame intra blocks across skipped and non-skipped residual blocks, segmented ALT_Q keyframes, segmented inter skip/ref-frame overrides with map reuse, direct non-scaled single-reference inter motion for NEWMV plus NEARESTMV/NEARMV candidate reuse, direct scaled compound NEWMV / NEARESTMV / NEARMV, and direct non-scaled compound NEWMV / NEARESTMV / NEARMV motion with distinct reference predictor averaging. Compound blocks are covered in both fixed compound-reference mode and reference-mode-select syntax, with LAST/ALTREF, GOLDEN/ALTREF, fixed-ALTREF, fixed-GOLDEN, and fixed-LAST sign-bias coverage. Interior subpel interpolation and libvpx-style edge extension for integer/subpel reference windows that cross the visible frame boundary are covered. Fixed eighttap, bilinear, and switchable per-block eighttap-smooth / eighttap-sharp filters are covered, and nonzero loop filtering is applied for supported keyframe/inter reconstructions. Edge-clipped zero-MV and tiled layouts are covered. Other valid VP9 frame classes still return `ErrVP9NotImplemented` at the reconstruction boundary. VP9 encoder construction / fixed-size `EncodeInto` are wired; the encoder currently emits a minimal keyframe path plus an intra-only inter fallback while the full prediction / residual pipeline is ported. |
...
| Decoder (uncompressed + compressed header, tile, detok, recon) | scaffolded: the public decoder parses uncompressed + compressed headers, initializes frame-context slots / loop-filter / dequant state, consumes keyframe + intra-only tile mode-info plus residual tokens, publishes reconstructed I420 output for 8-bit 4:2:0 intra frames, refreshes VP9 reference slots, can display stored show-existing frames without disturbing preserved header state, carries bitdepth / chroma sampling across inter headers, computes libvpx inter-mode contexts from nearest MV-ref neighbors, stores decoded inter MVs in the MI grid, performs the first MV-ref candidate scan for same-ref plus sign-bias-adjusted different-ref neighbors, reconstructs segmented ALT_Q keyframes through per-segment dequant, honors SEG_LVL_REF_FRAME + SEG_LVL_SKIP in inter mode parsing/reconstruction including copied prior segmentation maps, reconstructs inter-frame intra blocks through the intra predictor / residual path, reconstructs same-size single-reference inter blocks by copying zero-MV references or running direct inter prediction for NEWMV/NEARESTMV/NEARMV before inverse-transform-adding residual coefficients, reconstructs scaled-reference zero-MV / NEWMV / NEARESTMV / NEARMV inter blocks through libvpx scale-factor projection and scaled convolve dispatch, and reconstructs same-size plus scaled compound zero-MV, NEWMV, NEARESTMV, and NEARMV blocks by averaging the second predictor into the first. Interior and edge-extended integer/subpel/scaled windows dispatch through the ported VP9 convolve kernels for fixed eighttap, bilinear, and switchable per-block smooth/sharp filters. The supported 8-bit 4:2:0 reconstruction path now runs libvpx-style loop-filter mask traversal over keyframe and inter blocks. Segmentation, partition / mode drivers, MV helpers, coefficient detokenize, inverse-transform dispatch, intra/inter prediction helpers, compound reference syntax including reference-mode-select, scaled zero-MV/NEWMV/NEARESTMV/NEARMV references, LAST/ALTREF and GOLDEN/ALTREF compound pairs, all three compound fixed-reference sign-bias layouts for NEWMV, official VP90 IVF discovery / invalid-vector rejection, official profile-family WebM extraction with clean unsupported-profile rejection, and loop-filter traversal have unit coverage. Full frame-class/profile reconstruction, strict official VP90 IVF pixel parity, and full WebM-contained VP9 pixel parity are still open. |
...
| Oracle harness (vp90 IVF corpus + vpxenc/vpxdec gates) | DSP oracle complete: `internal/coracle/build_libvpx_vp9.sh` builds a VP9-decoder+encoder libvpx variant + the DSP oracle binary, which emits a deterministic 1415-record byte-parity corpus checked in under `internal/vp9/dsp/testdata/dsp_oracle.bin`. `internal/coracle/build_vpxdec_vp9.sh` builds a VP9-enabled vpxdec/vpxenc pair; root VP9 encoder tests wrap emitted payloads in VP90 IVF and gate single-SB, horizontal multi-SB, vertical multi-SB, 4x3-SB, edge-clipped/sub-SB keyframes, matching intra-only inter fallback frames, and skipped inter frames through vpxdec. `fetch-vp9-test-data` fetches the pinned official VP9 decoder subset for direct IVF and profile-family WebM coverage: 7 valid VP90 IVF streams, 17 invalid VP9 IVF streams including the invalid VP91 mixed-CSP vector, and 11 valid VP91/VP92/VP93 WebM profile vectors. The default oracle gate validates invalid-vector rejection, validates that official unsupported profile-family WebM streams are rejected as `ErrVP9NotImplemented` instead of malformed data, and skips valid official VP90 IVF streams that still need unsupported decoder features unless `GOVPX_VP9_TEST_DATA_STRICT=1` is set. The production oracle gate now runs those VP9 vpxdec structural tests plus VP9 decoder I420 byte-parity checks against libvpx vpxdec for supported nonzero-residue intra, segmented ALT_Q intra, show-existing, skipped zero-MV inter, scaled zero-MV/NEWMV/NEARESTMV/NEARMV inter, segmented ALTREF/SKIP inter including map reuse, zero-MV inter-residual, compound zero-MV inter, compound integer/subpel NEWMV with distinct refs including GOLDEN/ALTREF and all fixed-reference sign-bias layouts, scaled compound NEWMV/NEARESTMV/NEARMV, compound reference-mode-select NEWMV, compound NEARESTMV/NEARMV reuse, compound bilinear and switchable-smooth subpel NEWMV, inter-frame intra skip/residual, loop-filtered keyframe / inter-residual / inter-NEWMV, direct integer/subpel NEWMV, NEARESTMV/NEARMV reuse, bilinear, switchable smooth/sharp, and top/right border-extended NEWMV streams, including sub-SB / right-edge / bottom-edge / corner-edge residual fixtures. |
| Byte parity gate | DSP kernels: 1415/1415 records byte-identical to libvpx v1.16.0 (inverse transforms + intra + convolve + loop filter + SAD + variance). VP9 decoder output is byte-identical to libvpx vpxdec for the currently supported 8-bit 4:2:0 intra residual/segmented intra/show-existing/skipped zero-MV inter/scaled zero-MV/NEWMV/NEARESTMV/NEARMV inter/segmented ref-skip inter/inter-residual/compound zero-MV inter/compound NEWMV/NEARESTMV/NEARMV inter/scaled compound NEWMV/NEARESTMV/NEARMV/inter-frame intra/loop-filtered keyframe and inter/direct integer-subpel NEWMV/NEARESTMV/NEARMV/filter-mode/border-extension streams covered by `TestVP9DecoderVpxdecOracleMatches*`. Official VP90 IVF discovery, invalid rejection, and profile-family WebM unsupported rejection are gated, with strict official valid-vector pixel parity still open. Encoder bitstreams are currently structural-parity gated by vpxdec acceptance for the minimal skip=1 DC path; full encoded-packet byte identity and full decoded-pixel parity are still open. |
| Hot-path allocation guards | bitstream Reader.Read / Writer.Write, idct4x4 / idct8x8 / idct16x16 hot paths, VP9Encoder `EncodeInto` steady state after setup, and VP9Decoder `Decode` / `DecodeInto` header + tile/residual parse plus intra, segmented ALT_Q keyframe, segmented ALTREF/SKIP inter with map reuse, scaled zero-MV/NEWMV/NEARESTMV/NEARMV inter, scaled compound NEWMV/NEARESTMV/NEARMV inter, loop-filtered keyframe, inter-frame intra, compound zero-MV inter, compound GOLDEN/ALTREF NEWMV inter, compound sign-bias NEWMV layouts, compound subpel NEWMV inter, compound NEARMV inter, zero-MV inter-residual, loop-filtered inter residual/motion, direct integer/subpel NEWMV, border-extended subpel NEWMV, switchable subpel NEWMV, and NEARESTMV reconstruct output steady state after setup. |
| SIMD/assembly | not started |
