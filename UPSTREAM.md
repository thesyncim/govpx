...
| Encoder rate-control API | target bits, buffer model, libvpx one-pass keyframe target sizing/boosts, libvpx-style droppable encoded-frame metadata, max-intra-bitrate keyframe target cap, opt-in CBR golden-frame boost, libvpx bits-per-macroblock quantizer regulation plus bounded correction-factor feedback for one-pass CBR frames, libvpx frame-size bounds for bounded pre-commit quantizer feedback, post-frame quantizer feedback, bounded CBR frame dropping, CQLevel constrained-quality quantizer floor/control, and deterministic clip-level bitrate tracking tests scaffolded |
