...
port; VP9 status is tracked separately below. VP9 scope is deliberately
limited to full profile 0 support: 8-bit 4:2:0 bitstreams, including valid
superframes. Profiles 1, 2, and 3, high bit depth, non-4:2:0 chroma, alpha,
and container behavior are excluded from the VP9 target.
...
The VP9 port is in progress on the `vp9-port` branch. The target is complete
VP9 profile 0 parity and nothing outside profile 0. Valid non-profile0
bitstreams are parsed far enough to reject cleanly as unsupported, but
profile-family reconstruction is not tracked as future work. Tracking layout:

| Subsystem | Status |
| --- | --- |
| Public API (VP9Decoder / VP9Encoder) | scaffolded. VP9 decoder construction / option validation, `Decode`, `DecodeWithPTS`, `DecodeInto`, `LastFrameInfo`, `Reset`, and `NextFrame` are scoped to full profile 0 packets only. The decoder handles raw profile 0 payloads and valid superframe indexes, publishes I420 profile 0 output, carries PTS metadata, supports caller-owned output planes, configured resolution-change rejection, VP9's four frame-context slots / `refresh_frame_context` gate, reference refresh, show-existing display, profile 0 intra/inter reconstruction, segmentation, compound reference syntax, scaled references, motion-vector reuse, subpel interpolation, edge extension, tiling, loop filtering, and probability adaptation paths currently covered by the oracle tests. Valid profiles 1, 2, and 3 return `ErrVP9NotImplemented` by design, not as a pending implementation gap. VP9 encoder construction / fixed-size `EncodeInto` are wired for the profile 0 path; the encoder currently emits a minimal keyframe path plus an intra-only inter fallback while the full profile 0 prediction / residual pipeline is ported. |
...
| Decoder (uncompressed + compressed header, tile, detok, recon) | scaffolded for the profile 0 target: the public decoder parses superframe indexes, uncompressed + compressed headers, initializes frame-context slots / loop-filter / dequant state, consumes tile mode-info plus residual tokens, publishes reconstructed profile 0 I420 output, refreshes VP9 reference slots, displays stored show-existing frames without disturbing preserved header state, computes libvpx inter-mode contexts from nearest MV-ref neighbors, stores decoded inter MVs in the MI grid, performs MV-ref candidate scans, reconstructs segmented ALT_Q keyframes, honors SEG_LVL_REF_FRAME + SEG_LVL_SKIP in inter mode parsing/reconstruction including copied prior segmentation maps, reconstructs inter-frame intra blocks, reconstructs same-size and scaled single-reference inter blocks, reconstructs same-size and scaled compound blocks, runs fixed eighttap, bilinear, and switchable smooth/sharp subpel filters, performs libvpx-style edge extension, and runs loop-filter mask traversal over keyframe and inter blocks. Segmentation, partition / mode drivers, MV helpers, coefficient detokenize/count adaptation, inverse-transform dispatch, intra/inter prediction helpers, compound reference syntax including reference-mode-select, scaled references, official VP90 IVF discovery / invalid-vector rejection, profile-family WebM unsupported rejection, and loop-filter traversal have unit coverage. Remaining decoder work is closing profile 0 parity gaps, especially strict official VP90 IVF pixel parity and full WebM-contained profile 0 pixel parity. Non-profile0 reconstruction is excluded. |
...
| Oracle harness (vp90 IVF corpus + vpxenc/vpxdec gates) | DSP oracle complete: `internal/coracle/build_libvpx_vp9.sh` builds a VP9-decoder+encoder libvpx variant + the DSP oracle binary, which emits a deterministic 1415-record byte-parity corpus checked in under `internal/vp9/dsp/testdata/dsp_oracle.bin`. `internal/coracle/build_vpxdec_vp9.sh` builds a VP9-enabled vpxdec/vpxenc pair; root VP9 encoder tests wrap emitted payloads in VP90 IVF and gate single-SB, horizontal multi-SB, vertical multi-SB, 4x3-SB, edge-clipped/sub-SB keyframes, matching intra-only inter fallback frames, and skipped inter frames through vpxdec. `fetch-vp9-test-data` fetches the pinned official VP9 decoder subset for direct profile 0 IVF coverage, invalid VP9 IVF coverage, and profile-family WebM rejection coverage. The default oracle gate validates invalid-vector rejection, validates that official non-profile0 WebM streams are rejected as `ErrVP9NotImplemented` instead of malformed data, and skips valid official VP90 IVF streams that still need profile 0 decoder work unless `GOVPX_VP9_TEST_DATA_STRICT=1` is set. The production oracle gate now runs those VP9 vpxdec structural tests plus VP9 decoder I420 byte-parity checks against libvpx vpxdec for supported profile 0 reconstruction paths. |
| Byte parity gate | DSP kernels: 1415/1415 records byte-identical to libvpx v1.16.0 (inverse transforms + intra + convolve + loop filter + SAD + variance). VP9 decoder output is byte-identical to libvpx vpxdec for the profile 0 reconstruction streams covered by `TestVP9DecoderVpxdecOracleMatches*`. Official VP90 IVF discovery, invalid rejection, and non-profile0 WebM unsupported rejection are gated, with strict official profile 0 valid-vector pixel parity still open. Encoder bitstreams are currently structural-parity gated by vpxdec acceptance for the minimal profile 0 skip=1 DC path; full profile 0 encoded-packet byte identity and full decoded-pixel parity are still open. |
