...
- `EncodeInto` can emit source-dependent whole-block luma/chroma intra keyframes,
  ...
  default reconstructed-frame loop filtering, and initial CQLevel constrained-quality mode. One-pass CBR/CQ/VBR now start frames from libvpx's bits-per-macroblock quantizer regulator, update key/inter/golden-frame correction factors from encoded frame size, and track the current external 90p vpxenc validation clip closely, but full libvpx
