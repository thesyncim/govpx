...
| Hot-path allocation guards | bitstream Reader.Read / Writer.Write, idct4x4 / idct8x8 / idct16x16 hot paths, VP9Encoder `EncodeInto` steady state after setup, and VP9Decoder `Decode` / `DecodeInto` header + tile/residual parse plus intra, zero-MV inter-residual, direct integer/subpel NEWMV, and NEARESTMV reconstruct output steady state after setup. |
| SIMD/assembly | not started |
