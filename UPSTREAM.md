...
| IVF/test vectors | IVF parser and oracle checksum parser scaffolded; normal-test VP8/IVF smoke vectors, including libvpx-authored reference, token-partition, profile, loop-filter sharpness, and error-resilient streams plus NEWMV, subpixel NEWMV, luma/chroma intra-mode keyframe, and intra-macroblock interframe coverage, checksummed against libvpx v1.16.0; opt-in encoder oracle coverage includes B_PRED keyframes, intra B_PRED interframes, CQLevel streams, libvpx decode-acceptance/checksum agreement for both govpx-encoded and libvpx-encoded corpus clips, token-partition and segmentation feature assertions, aggregate and per-frame PSNR/SSIM floors, bitrate windows, and libvpx-vpxenc quality-gap guards; opt-in generated libvpx corpus covers profile 0/1/2/3, one/two/four/eight token partitions, active eight-partition row cycling, error-resilient cyclic-refresh segmentation, sharpness, and static-threshold streams with feature assertions; production external VP8 IVF decoder conformance requires the full valid VP8 subset currently available through the pinned libvpx v1.16.0 test-data list (58 VP80 IVF vectors plus 2 invalid rejection vectors; four non-invalid `vp80-03-segmentation-0*.ivf` fixtures are I420 IVF resources), with invalid-stream rejection parity via `GOVPX_INVALID_TEST_DATA_PATH` and external Y4M/YUV encoder source-corpus validation via `GOVPX_ENCODER_TEST_DATA_PATH`, all with required/minimum-count controls for CI corpus runs |
...
| Encoder rate-control API | target bits, buffer model, max-intra-bitrate keyframe target cap, opt-in CBR golden-frame boost, libvpx bits-per-macroblock quantizer regulation for one-pass CBR frames, bounded pre-commit quantizer feedback, post-frame quantizer feedback, bounded CBR frame dropping, CQLevel constrained-quality quantizer floor/control, and deterministic clip-level bitrate tracking tests scaffolded |
...
- `EncodeInto` can emit source-dependent whole-block luma/chroma intra keyframes,
  ...
  default reconstructed-frame loop filtering, and initial CQLevel constrained-quality mode. One-pass CBR now starts frames from libvpx's bits-per-macroblock quantizer regulator and tracks the current external 90p vpxenc validation clip closely, but full libvpx
