...
| Encoder rate-control API | target bits, buffer model, max-intra-bitrate keyframe target cap, opt-in CBR golden-frame boost, bounded pre-commit quantizer feedback, post-frame quantizer feedback, bounded CBR frame dropping, CQLevel constrained-quality quantizer floor/control, and deterministic clip-level bitrate tracking tests scaffolded |
...
- `EncodeInto` can emit source-dependent whole-block luma/chroma intra keyframes,
  ...
  default reconstructed-frame loop filtering, and initial CQLevel constrained-quality mode, but full libvpx
