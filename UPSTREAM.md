...
| Subsystem | Status |
| --- | --- |
| Public API | scaffolded; decoder options expose libvpx-style error concealment and granular postprocess controls; runtime encoder controls cover bitrate, rate-control config, CQLevel, max-intra bitrate, GF-CBR boost, token partitions, sharpness, screen-content mode, static threshold, noise sensitivity, ARNR controls, two-pass stats, realtime targets, temporal scalability, temporal layer ID, deadline/CPU-used, keyframe interval, adaptive keyframes, force-keyframe, force-Golden/AltRef frame flags, flush, and reset; encoder results report libvpx-style temporal layer IDs, sync, TL0PICIDX, droppable metadata, lookahead depth, preprocessing flags, scene-cut keyframe promotion, first-pass stats, two-pass frame targets, incremental/cumulative layer bitrate targets, per-layer buffer state, and per-layer input/encoded/cumulative bit counters; zero-allocation hot-path coverage exists for runtime controls |
| Upstream manifest | scaffolded |
| Oracle harness | `make verify-production` builds pinned libvpx helpers, including vpxenc, vpxdec, and the VP8 temporal SVC example encoder, fetches required corpora, and runs the full oracle gate; `make verify-decoder-parity` runs the decoder-only oracle proof gate; optional vpxdec smoke and libvpx checksum oracle scaffolded, including postprocess, ADDNOISE, and error-concealment decode modes for targeted parity tests |
...
| IVF/test vectors | IVF parser and oracle checksum parser scaffolded; normal-test VP8/IVF smoke vectors, including libvpx-authored reference, token-partition, profile, loop-filter sharpness, and error-resilient streams plus NEWMV, subpixel NEWMV, luma/chroma intra-mode keyframe, and intra-macroblock interframe coverage, checksummed against libvpx v1.16.0; decoder parity proof requires the full valid VP8 subset currently available through the pinned libvpx v1.16.0 test-data list (58 VP80 IVF vectors plus 2 invalid rejection vectors; four non-invalid `vp80-03-segmentation-0*.ivf` fixtures are I420 IVF resources), checksums both `Decode` and `DecodeInto` against libvpx, and includes generated libvpx feature streams for profile 0/1/2/3, one/two/four/eight token partitions, active eight-partition row cycling, error-resilient cyclic-refresh segmentation, sharpness, static-threshold streams, postprocess, error-concealment, and keyframe resolution-change coverage; opt-in encoder oracle coverage includes B_PRED keyframes, intra B_PRED interframes, temporal SVC base-layer and full-sequence streams, external libvpx temporal SVC example layer streams plus layer accounting checks, CQLevel streams, libvpx decode-acceptance/checksum agreement for both govpx-encoded and libvpx-encoded corpus clips, token-partition and segmentation feature assertions, aggregate and per-frame PSNR/SSIM floors, bitrate windows, and libvpx-vpxenc quality-gap guards; normal temporal SVC coverage checks packet refresh/entropy flags across all libvpx example patterns; production external Y4M/YUV encoder source-corpus validation uses `GOVPX_ENCODER_TEST_DATA_PATH` with required/minimum-count controls for CI corpus runs |
