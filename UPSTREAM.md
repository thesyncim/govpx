...
| Subsystem | Status |
| --- | --- |
| Public API | scaffolded; decoder options expose libvpx-style error concealment and granular postprocess controls; runtime encoder controls cover bitrate, rate-control config, CQLevel, max-intra bitrate, GF-CBR boost, token partitions, sharpness, screen-content mode, static threshold, realtime targets, temporal scalability, temporal layer ID, deadline/CPU-used, keyframe interval, adaptive keyframes, force-keyframe, force-Golden/AltRef frame flags, and reset; encoder results report libvpx-style temporal layer IDs, sync, TL0PICIDX, droppable metadata, scene-cut keyframe promotion, incremental/cumulative layer bitrate targets, per-layer buffer state, and per-layer input/encoded/cumulative bit counters; zero-allocation hot-path coverage exists for runtime controls |
...
- `EncodeInto` can emit source-dependent whole-block luma/chroma intra keyframes,
  ...
  default reconstructed-frame loop filtering, opt-in one-pass scene-cut keyframe
