...
func FuzzVP8EncoderOptions(f *testing.F) {
	...
	seeds := [][]byte{
		...
		// noise]. rcPool = {CBR, VBR, CQ, Q}; dlPool = {RT, Good,
		// Best}; cpuPool = {0, -3, 4, 8}; widthPool = heightPool =
		// {16, 32, 48, 64}.
		// 64x64 CQ cq20 good cpu=0 1000kbps.
		{0x03, 0x03, 0x02, 0x01, 0x00, 0x04, 0x38, 0x14, 0x2c, 0x01, 0xb6, 0x03, 0x00, 0x00, 0x01, 0x00},
		// 64x64 Q cq40 good cpu=0 1000kbps.
		{0x03, 0x03, 0x03, 0x01, 0x00, 0x04, 0x38, 0x28, 0x2c, 0x01, 0xb6, 0x03, 0x00, 0x00, 0x01, 0x00},
		// 48x48 CQ cq56 best cpu=0 200kbps (high-CQ regime, exercises
		// active_worst CQ floor in libvpxActiveWorstQuantizerForFrame).
		{0x02, 0x02, 0x02, 0x02, 0x00, 0x04, 0x70, 0x38, 0x2c, 0x01, 0x96, 0x00, 0x00, 0x00, 0x01, 0x00},
		// 32x32 Q cq4 good cpu=0 1000kbps (low-CQ near minQ).
		{0x01, 0x01, 0x03, 0x01, 0x00, 0x04, 0x38, 0x04, 0x2c, 0x01, 0xb6, 0x03, 0x00, 0x00, 0x01, 0x00},
		// 64x64 CQ cq30 realtime cpu=-3 1000kbps (exercises the
		// realtime recode-loop=0 CQ path).
		{0x03, 0x03, 0x02, 0x00, 0x01, 0x04, 0x38, 0x1e, 0x2c, 0x01, 0xb6, 0x03, 0x00, 0x00, 0x01, 0x00},
		// 48x48 CQ cq20 with sharpness=4 good cpu=4.
		{0x02, 0x02, 0x02, 0x01, 0x02, 0x04, 0x38, 0x14, 0x2c, 0x01, 0xb6, 0x03, 0x04, 0x00, 0x01, 0x00},
	}
