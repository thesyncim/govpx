...
func (e *VP9Encoder) encodeVP9FrameIntoWithFlagsResultInternal(img *image.YCbCr, dst []byte, flags EncodeFlags, forceIntraOnly bool, temporalFrame temporalFrame, forceInterLayerRefresh bool, isSrcFrameAltRef bool) (result VP9EncodeResult, err error) {
	...
	if !isKey && !intraOnly && showFrame && !e.rc.highSourceSAD {
		...
		if dropFrame {
			...
			return VP9EncodeResult{
				...
				TemporalLayeringMode:        e.temporal.config.Mode,
	...
	*header = vp9dec.UncompressedHeader{
	...
	header.Tile = vp9EncoderTileInfoForTargetLevel(miCols, int(width), int(height),
		e.vp9TileWorkerThreadHint(), e.opts.Log2TileRows, e.opts.TargetLevel)
