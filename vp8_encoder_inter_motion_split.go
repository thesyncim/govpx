...
func selectInterFrameSplitBlockFullPixelMotionVectorWithBounds(src vp8enc.SourceImage, ref *vp8common.Image, mbRow int, mbCol int, block int, width int, height int, searchCenter vp8enc.MotionVector, bestRefMV vp8enc.MotionVector, qIndex int, errorPerBit int, stepParam int, fullSearchFallback bool, mvProbs *[2][vp8tables.MVPCount]uint8, mvCosts *vp8enc.MotionVectorCostTables, bounds vp8enc.InterFrameFullPixelBounds) (vp8enc.MotionVector, int) {
	...
	search := newSplitFullPelSearchCtx(src, ref, mbRow, mbCol, block, width, height)
	refRowFull := int(bestRefMV.Row) >> 3
	refColFull := int(bestRefMV.Col) >> 3
	centerWalkCost := search.fullPelCostFull(int(best.Row)>>3, int(best.Col)>>3, refRowFull, refColFull, qIndex)
	...
	best, bestReturnCost := nstepInterFrameSplitBlockFullPixelMotionVectorWithErrorPerBitAndCostTables(&search, best, centerWalkCost, centerReturnCost, bestRefMV, refRowFull, refColFull, qIndex, errorPerBit, mvProbs, mvCosts, bounds, stepParam)
	if fullSearchFallback && splitMotionFullSearchFallbackNeeded(bestReturnCost, width, height) {
		candidate := fullSearchInterFrameSplitBlockFullPixelMotionVectorWithErrorPerBitAndCostTables(&search, bounds.ClampEighth(vp8enc.MotionVector{Row: int16(centerRow), Col: int16(centerCol)}), bestRefMV, refRowFull, refColFull, qIndex, errorPerBit, mvProbs, mvCosts, bounds, interFrameFullPixelSearchRadius)
...
func nstepInterFrameSplitBlockFullPixelMotionVectorWithErrorPerBitAndCostTables(search *splitFullPelSearchCtx, center vp8enc.MotionVector, centerWalkCost int, centerReturnCost int, bestRefMV vp8enc.MotionVector, refRowFull int, refColFull int, qIndex int, errorPerBit int, mvProbs *[2][vp8tables.MVPCount]uint8, mvCosts *vp8enc.MotionVectorCostTables, bounds vp8enc.InterFrameFullPixelBounds, stepParam int) (vp8enc.MotionVector, int) {
	stepParam = min(max(stepParam, 0), interFrameMaxMVSearchSteps-1)
	furtherSteps := (interFrameMaxMVSearchSteps - 1) - stepParam
	result := diamondNstepInterFrameSplitBlockFullPixelMotionVectorWithErrorPerBitAndCostTables(search, center, centerWalkCost, centerReturnCost, bestRefMV, refRowFull, refColFull, qIndex, errorPerBit, mvProbs, mvCosts, bounds, stepParam)
	best := result.mv
	...
	for n < furtherSteps {
		...
		candidate := diamondNstepInterFrameSplitBlockFullPixelMotionVectorWithErrorPerBitAndCostTables(search, center, centerWalkCost, centerReturnCost, bestRefMV, refRowFull, refColFull, qIndex, errorPerBit, mvProbs, mvCosts, bounds, stepParam+n)
...
func diamondNstepInterFrameSplitBlockFullPixelMotionVectorWithErrorPerBitAndCostTables(search *splitFullPelSearchCtx, center vp8enc.MotionVector, centerWalkCost int, centerReturnCost int, bestRefMV vp8enc.MotionVector, refRowFull int, refColFull int, qIndex int, errorPerBit int, mvProbs *[2][vp8tables.MVPCount]uint8, mvCosts *vp8enc.MotionVectorCostTables, bounds vp8enc.InterFrameFullPixelBounds, searchParam int) splitFullPixelSearchResult {
	...
	for range totalSteps {
		for range 8 {
			...
			if bounds.ContainsFullPelStrict(row, col) {
				cost := search.fullPelCostFull(row, col, refRowFull, refColFull, qIndex)
	...
	if best != center {
		var ok bool
		returnCost, ok = interMotionSplitBlockFullPixelReturnCostWithErrorPerBitAndCostTables(search.src, search.ref, search.mbRow, search.mbCol, search.block, search.width, search.height, best, bestRefMV, errorPerBit, mvProbs, mvCosts)
		if !ok {
			returnCost = maxInt()
		}
	}
...
func fullSearchInterFrameSplitBlockFullPixelMotionVectorWithErrorPerBitAndCostTables(search *splitFullPelSearchCtx, center vp8enc.MotionVector, bestRefMV vp8enc.MotionVector, refRowFull int, refColFull int, qIndex int, errorPerBit int, mvProbs *[2][vp8tables.MVPCount]uint8, mvCosts *vp8enc.MotionVectorCostTables, bounds vp8enc.InterFrameFullPixelBounds, distance int) splitFullPixelSearchResult {
	...
	best := center
	bestWalkCost := search.fullPelCostFull(int(best.Row)>>3, int(best.Col)>>3, refRowFull, refColFull, qIndex)
...
func stepInterFrameSplitBlockSubpixelMotionVector(src vp8enc.SourceImage, ref *vp8common.Image, mbRow int, mbCol int, block int, width int, height int, best vp8enc.MotionVector, bestRefMV vp8enc.MotionVector, qIndex int, errorPerBit int, quarter bool, mvProbs *[2][vp8tables.MVPCount]uint8, mvCosts *vp8enc.MotionVectorCostTables) (vp8enc.MotionVector, int, bool) {
	...
	bestCost, bestRow, bestCol = stepInterFrameSplitBlockSubpixelDirectionalSearch(&cache, src, ref, mbRow, mbCol, block, width, height, bestRow, bestCol, 2, bestCost, bestRefMV, errorPerBit, mvProbs, mvCosts)
	if quarter {
		bestCost, bestRow, bestCol = stepInterFrameSplitBlockSubpixelDirectionalSearch(&cache, src, ref, mbRow, mbCol, block, width, height, bestRow, bestCol, 1, bestCost, bestRefMV, errorPerBit, mvProbs, mvCosts)
	}
...
func stepInterFrameSplitBlockSubpixelDirectionalSearch(cache *subpelEvalCache, src vp8enc.SourceImage, ref *vp8common.Image, mbRow int, mbCol int, block int, width int, height int, startRow int, startCol int, step int, bestCost int, bestRefMV vp8enc.MotionVector, errorPerBit int, mvProbs *[2][vp8tables.MVPCount]uint8, mvCosts *vp8enc.MotionVectorCostTables) (int, int, int) {
	bestRow := startRow
	bestCol := startCol
	leftCost := cachedSplitBlockSubpixelMotionSearchCandidateCost(cache, src, ref, mbRow, mbCol, block, width, height, startRow, startCol-step, bestRefMV, errorPerBit, mvProbs, mvCosts)
	rightCost := cachedSplitBlockSubpixelMotionSearchCandidateCost(cache, src, ref, mbRow, mbCol, block, width, height, startRow, startCol+step, bestRefMV, errorPerBit, mvProbs, mvCosts)
	upCost := cachedSplitBlockSubpixelMotionSearchCandidateCost(cache, src, ref, mbRow, mbCol, block, width, height, startRow-step, startCol, bestRefMV, errorPerBit, mvProbs, mvCosts)
	downCost := cachedSplitBlockSubpixelMotionSearchCandidateCost(cache, src, ref, mbRow, mbCol, block, width, height, startRow+step, startCol, bestRefMV, errorPerBit, mvProbs, mvCosts)
	...
	diagCost := cachedSplitBlockSubpixelMotionSearchCandidateCost(cache, src, ref, mbRow, mbCol, block, width, height, diagRow, diagCol, bestRefMV, errorPerBit, mvProbs, mvCosts)
...
func iterativeInterFrameSplitBlockSubpixelMotionVector(src vp8enc.SourceImage, ref *vp8common.Image, mbRow int, mbCol int, block int, width int, height int, best vp8enc.MotionVector, bestRefMV vp8enc.MotionVector, qIndex int, errorPerBit int, mvProbs *[2][vp8tables.MVPCount]uint8, mvCosts *vp8enc.MotionVectorCostTables) (vp8enc.MotionVector, int, bool) {
	...
	cand := func(r, c int) int {
		return cachedBoundedSplitBlockSubpixelMotionSearchCandidateCost(&cache, bounds, src, ref, mbRow, mbCol, block, width, height, r, c, bestRefMV, errorPerBit, mvProbs, mvCosts)
	}
...
func cachedSplitBlockSubpixelMotionSearchCandidateCost(cache *subpelEvalCache, src vp8enc.SourceImage, ref *vp8common.Image, mbRow int, mbCol int, block int, width int, height int, row int, col int, bestRefMV vp8enc.MotionVector, errorPerBit int, mvProbs *[2][vp8tables.MVPCount]uint8, mvCosts *vp8enc.MotionVectorCostTables) int {
	if eval, ok := cache.get(row, col); ok {
		return eval.cost
	}
	eval := subpelCandidateEval{cost: maxInt()}
	if cost, ok := splitBlockSubpixelMotionSearchCandidateCostWithCostTables(src, ref, mbRow, mbCol, block, width, height, row, col, bestRefMV, errorPerBit, mvProbs, mvCosts); ok {
		eval = subpelCandidateEval{cost: cost, ok: true}
	}
	cache.put(row, col, eval)
	return eval.cost
}

func cachedBoundedSplitBlockSubpixelMotionSearchCandidateCost(cache *subpelEvalCache, bounds vp8enc.InterFrameSubpelSearchBounds, src vp8enc.SourceImage, ref *vp8common.Image, mbRow int, mbCol int, block int, width int, height int, row int, col int, bestRefMV vp8enc.MotionVector, errorPerBit int, mvProbs *[2][vp8tables.MVPCount]uint8, mvCosts *vp8enc.MotionVectorCostTables) int {
	if eval, ok := cache.get(row, col); ok {
		return eval.cost
	}
	eval := subpelCandidateEval{cost: maxInt()}
