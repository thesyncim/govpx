...
// TestVP8Task362SubPelSearchGatePreservesNegativeCPUUsedCascade pins that
...
// interAnalysisSearchConfig.fractionalSearch dispatch as before
// task #362, matching libvpx's `Speed > 4 → Step`, `Speed > 8 → Half`,
// `Speed >= 15 → Skip` cascade at vp8/encoder/onyx_if.c:954/1012/1023.
//
// The targeted-gate pattern (mirrors task #350
// libvpxRealtimeCPISpeedForImprovedMVPredGate) only kicks in when
...
// dispatch — guarding the task #247
// (TestVP8Task247ExtremeCPUUsedHalfPixelStaysEnabled) and the broader
// TestVP8SpeedFeaturesPickInterMirrorsLibvpxRTSpeedCascade pins.
func TestVP8Task362SubPelSearchGatePreservesNegativeCPUUsedCascade(t *testing.T) {
	cases := []struct {
		cpiSpeed int
...
// TestVP8Task362SubPelSearchGateColdStartUnchanged pins that the new
...
// start dispatch keeps the keyframe encode aligned with the task #278
// inter-frame wall-clock pin.
func TestVP8Task362SubPelSearchGateColdStartUnchanged(t *testing.T) {
...
// TestVP8Task362SubPelSearchGateRealisticSpeedAfterColdStart pins that
...
// 16). This is the audit-observed trajectory from the task #343 720p
// RT cpu=8 per-frame trace (cpi_speed=9 at frame 2). When the libvpx-
...
// pel dispatch, leaving every other speed-feature lookup on the actual
// e.autoSpeed.
func TestVP8Task362SubPelSearchGateRealisticSpeedAfterColdStart(t *testing.T) {
	cases := []struct {
		cpuUsed       int
		wantSpeed     int
		...
		// the task #343/#350 audit-observed cpi_speed=9 trajectory.
...
// TestVP8Task362SubPelSearchGateCPU0RTUnchanged pins that cpu_used==0
...
func TestVP8Task362SubPelSearchGateCPU0RTUnchanged(t *testing.T) {
...
// TestVP8Task362SubPelSearchGateNonRTUnchanged pins that non-realtime
...
func TestVP8Task362SubPelSearchGateNonRTUnchanged(t *testing.T) {
