...
func TestProductionGateReportsVP9BrowserStallBudgets(t *testing.T) {
	...
	for _, want := range []string{
		...
		"browser-plain-vp9-720p-threaded-loaded",
		...
		"TestPlainVP9WebRTCPacketizerRecoveryAfter(PacketizedUnsent|PartialWrite)AccessUnits",
		`GOVPX_WITH_ORACLE: "1"`,
		"requiresOracle: true",
		"assertNoOracleSkips",
...
func TestStressGateReportsVP9HostileSoakBudgets(t *testing.T) {
	...
	for _, want := range []string{
		...
		"browser-plain-vp9-temporal-loaded-withhold-soak",
		"browser-plain-vp9-temporal-loaded-partial-write-soak",
