...
go run . -mode server -addr :8090 -public-url http://SERVER_HOST:8090
```

The server prints a pairing URL and a QR code in the terminal. It also serves a
pairing page at:
...
The QR encodes a short-lived one-time `/pair/<token>` URL. Scan it with the
client device camera; on iOS this opens Safari directly into the browser client.

For a native Gio client, pass the same pairing URL:

```sh
cd examples/gio-webrtc-remote
go run . -mode client -server http://SERVER_HOST:8090/pair/TOKEN_FROM_QR_PAGE
```

Client mode is the default, so `go run . -server ...` is equivalent. Click
inside the remote view to focus it; pointer movement, clicks, scrolls, and keys
are sent over the WebRTC data channel and acknowledged in the footer. The native
client header shows decoded FPS.
...
For local development without token pairing, start the server with
`-require-pairing=false` and open `/client` or point the native client at the
server root.

## Pairing model

- Pairing tokens are generated with `crypto/rand`.
- Tokens are URL-safe, short-lived, and one-time use.
- `/` shows the current QR code and pairing URL.
- `/qr` serves the QR code as a PNG generated by a pure-Go encoder.
- `/pair/<token>` serves the browser client without consuming the token.
- `/offer?token=<token>` consumes the token and starts the WebRTC session.

This keeps the QR payload small: it contains only the pairing URL, not bulky SDP.
The browser or native client still creates the SDP offer at connect time.
...
- A browser client can display the same VP8 WebRTC stream through the browser's
  native decoder.
- A phone can pair by scanning a pure-Go-generated QR code and then use the same
