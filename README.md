...
Encode comparison runs default to autospeed calibration so govpx follows the
same libvpx-equivalent speed-feature trajectory while timing the actual Go
encoder. Pass `-autospeed-calibration=false` to inspect production wall-clock
autospeed behavior separately.
