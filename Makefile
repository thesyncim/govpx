...
test-oracle: oracle-tools vp9-vpxdec-tools fetch-test-data
	...
	$(GO) test . -run 'Test(VP8Oracle|Oracle|VP9EncoderVpxdecOracleAccepts|VP9DecoderVpxdecOracleMatches|VP9DecoderOfficial|VP9DecoderThreadingOfficial)' -count=1 -timeout 10m
...
oracle-tools: $(ORACLE)
	...
	sh internal/coracle/build_vpxenc_frameflags_oracle.sh >/dev/null
	test -x "$(VPXENC)"
	test -x "$(VPXENC_ORACLE)"
	test -x "$(VPXENC_FRAMEFLAGS)"
	test -x "$(VPXENC_FRAMEFLAGS_ORACLE)"
	test -x "$(VPXDEC)"
