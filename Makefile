...
ORACLE := $(CORACLE_BUILD)/gopx-vpx-oracle
...
oracle-test: oracle-tools fetch-test-data
	...
	LIBGOPX_ORACLE="$(ORACLE)" \
	LIBGOPX_VPXDEC="$(VPXDEC)" \
	LIBGOPX_VPXENC="$(VPXENC)" \
	LIBGOPX_TEST_DATA_PATH="$(VP8_TEST_DATA_DIR)" \
	LIBGOPX_TEST_DATA_REQUIRED=1 \
	LIBGOPX_TEST_DATA_MIN="$(VP8_DECODER_IVF_MIN)" \
	LIBGOPX_INVALID_TEST_DATA_REQUIRED=1 \
	LIBGOPX_INVALID_TEST_DATA_MIN="$(VP8_INVALID_IVF_MIN)" \
	LIBGOPX_ENCODER_TEST_DATA_PATH="$(VP8_ENCODER_SOURCE_DIR)" \
	LIBGOPX_ENCODER_TEST_DATA_REQUIRED=1 \
	LIBGOPX_ENCODER_TEST_DATA_MIN="$(VP8_ENCODER_SOURCE_MIN)" \
	LIBGOPX_ENCODER_TEST_DATA_FRAMES="$(VP8_ENCODER_SOURCE_FRAMES)" \
